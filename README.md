...
- `CampaignSiegePresentation.ts` samples ram strikes, oil pours and catapult fire/reload from confirmed operations on the zone clock. Catapults start in their cocked ready pose; wheel rotation follows travel distance. Repeated snapshots and LOD loads retain the current action phase. The in-memory authority review is `/authoring/blender/orvr-frontier/runtime-siege-review.html`; projectile/liquid effects and fitted crew remain separate unfinished work. Animation never applies damage.
