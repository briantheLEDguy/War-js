...
proof also runs with packaged Windows clients. Character import requires explicit
skeletal-material shader usage; proof lighting is dynamic to avoid unbuilt lightmaps. Complete graphical/equipment
...
world gathering and complete station workflows remain open. Native cultivation
supports both timed seeds, optional soil, three plots and capacity-safe harvest;
progression still needs durable persistence. `E` opens a nearby authored native
crafting station; `I` retains portable crafting. Campaign station placement and
...
a building in the world selects its registered authored collision volume.
Construction
...
The shared apothecary worktable is placed in all 36 keeps and both regional supply shelters and
...
- Cinderfen village cargo uses reviewed neutral crates, barrels, handcarts, washing lines and planters fitted to measured bounds by `scripts/campaign/cinderfen-dressing.mjs`. `cinderfen-ecology.mjs` places 634 reviewed reed/sedge clumps and 621 alders in shoreline colonies and twelve dry-margin stands. The retained terrain grid controls rooting, water bands and road/objective/patrol exclusions. All four Cinderfen nature assets, including fractured basalt, have three reviewed LODs and hash-matched GM defaults; 38 basalt outcrops replace the old scattered ridge rocks along eight surveyed shoulders and headlands. Woodland undergrowth and tree variation remain in production. The assembled review uses spatially batched exact exports and an Alder stand camera. Run `builder:generate` after map composition changes; consult `docs/overnight-world-work-state.md` for current production state.
- Sunmeadow's dwarf artisan revision l is published at three LODs with nine clips and assigned to the existing craft mentor. The corrected buck is placed with idle motion; fox, hare and wolf are rest-pose scenery, and skylark is rest-pose LOD0. Twelve wildlife placements stay off travel and objective corridors. These delivered assets are available in the GM builder; remaining regional inhabitants and fauna motion/LOD work are still tracked separately.
...
- Generic keep wall spans meet the actual scaled gate and door leaves. Their 5m outer approach narrows inside the forecourt to fit the inner doorway, including the road ribbon's feathered edge; supply roads still terminate at the delivery point. Sunmeadow and Cinderfen retain their surveyed road widths. `keepEnclosureCollision` covers all 36 keeps through closed, outer-breached and fully breached states, and `orvrRoadNetwork` checks complete road-width clearance.
- Local `Player.ts` divides horizontal movement into short collision steps so slow frames cannot carry the player through a thin gate, wall or rail. Cinderfen's corrected corner gangways move all 16 stair assemblies clear of the wall meshes; continuous elevated rail blockers follow the authored rail geometry. `cinderfenStairCollision` checks support, containment and placed-model evidence. The actual local player review is `/authoring/blender/orvr-frontier/runtime-keep-review.html`; measured browser results and remaining acceptance limits are recorded in [the September 20 checkpoint](docs/world-build-checkpoint-2026-09-20.md).
...
- `CampaignSiegePresentation.ts` samples ram strikes, oil pours and catapult fire/reload from confirmed operations on the zone clock. Catapults start in their cocked ready pose; wheel rotation follows travel distance. Repeated snapshots and LOD loads retain the current action phase. The in-memory authority review is `/authoring/blender/orvr-frontier/runtime-siege-review.html`; projectile/liquid effects and a full shared campaign siege playthrough remain unfinished. Animation never applies damage.
- `CampaignSiegeCrewPresentation.ts` applies the published left/right ram poses to compatible existing male Battle Prelate avatars. Authority-owned physical seats remain stable when an operator leaves, follow ram movement and turning, and use confirmed strike timing. Held weapons are hidden while operating and restored on dismount; voluntary exits require a clear traversal and ground landing. LOD changes preserve the pose, and actor disposal releases crew mixers. Actual equipped-player browser checks cover boarding, driving, turning, strike, all three LODs and weapon restoration. Other body families retain their existing avatars and require separate fit work.
- The current cauldron replacement is retained under `authoring/blender/orvr-frontier/cauldron-fit/`: six forged ornaments follow the bowl at every LOD, with exact original pour keys and sockets. Its scoped publication verifies actual contacts and reviewed exports; all three models remain available in the GM catalog. WebSocket regressions also verify confirmed siege timing on both clients, after reconnect, and through rejected cooldown attempts.
