...
def main():
    ...
    for lod,modules in all_modules.items():
        for obj in modules:
            ...
            if args.export:
                filename=("chr_civic_battle_prelate_t1_m" if slot=="body" else "wep_civic_battle_prelate_dawn_maul" if slot=="weapon" else f"arm_civic_battle_prelate_{slot}_t1_m")+("" if lod==0 else f"_lod{lod}")+".glb"
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/("battle_prelate_game_master.blend" if args.bake and args.export else "battle_prelate_rigged_study.blend")),compress=True)
