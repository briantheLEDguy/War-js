...
COLLECTION="REFERENCE_BUILT_EMBER"
PALETTE={
    "hair":((.15,.037,.017,1),0,.66), "ash_cloth":((.20,.174,.135,1),0,.91), "ember":((.85,.12,.008,1),0,.4),
...
def material(name):
    ...
    if name == "ember":
        shader.inputs["Emission Color"].default_value=(1,.12,.006,1)
...
def main():
    ...
    master_path=ROOT/("ember_arcanist_reference_master.blend" if scene_data.get("stage")=="full_character" else "ember_arcanist_upper_body_proof.blend")
    snapshots={p.name:p.read_bytes() for p in sorted((ROOT/"source").glob("*.json")) if p.name!="scene.json"}
    ...
    report={"schema_version":1,"build_status":"complete","stage":scene_data.get("stage","upper_body_proof"),"revision":scene_data["revision"],"build_id":build_id,"source_hashes":source_hashes,"visual_status":"unreviewed","runtime_promotion":False,
        ...
        "limitations":[
            "Full-character editable authored source; likeness and surface quality require visual review" if scene_data.get("stage")=="full_character" else "Upper-body editable source proof; the complete figure is outside this artifact",
