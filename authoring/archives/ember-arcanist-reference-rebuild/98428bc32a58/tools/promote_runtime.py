...
def allowed_destination(relative):
    ...
    return (len(parts) >= 6 and parts[:5] == ("public", "assets", "models", "reviews", "ember-arcanist-reference-rebuild")) or (
        len(parts) >= 5 and parts[:3] == ("authoring", "archives", "ember-arcanist-reference-rebuild"))
...
def provenance_source_paths(authoring_root, validation):
    ...
    paths.update({"source/CONTRACT.md", "source/FULL_CHARACTER_CONTRACT.md", "source/scene.json",
                  ...
                  "tools/author_ember.py", "tools/refine_ember.py", "tools/tessellate_runtime.py",
...
def module_manifest_metadata(manifest, slot):
    ...
    if slot == "weapon":
        ...
        manifest["displayName"] = "Ashbound Brazier Staff"
    return manifest
...
def prepare(repo, runtime, validation_path, review_path, node="node", authoring_root=ROOT):
    ...
    preview_root = MODELS / "reviews/ember-arcanist-reference-rebuild" / token
    ...
    archive_root = Path("authoring/archives/ember-arcanist-reference-rebuild") / token
    ...
    provenance = {"kind": "explicit_reference_mesh_rebuild", "geometrySource": "Literal authored vertices/faces; permitted finishing only",
                  "rigReuse": "Canonical armature/actions; explicit body/outfit cages adapted from Battle Prelate sources with new hair, collar, panels and staff",
    ...
    for slot in SLOTS:
        ...
        existing = repo / APPROVED / manifest_name
        manifest = read_json(existing if existing.exists() else authoring_root / "manifest_templates" / manifest_name)
        require(manifest.get("model") == model_name(slot, 0) and manifest.get("compatibility", {}).get("bodyVariant") == "m", "Target manifest does not match the male Ember Arcanist module")
...
def apply(plan_path, repo, authoring_root=ROOT):
    ...
    archive = within(repo, repo / "authoring/archives/ember-arcanist-promotions" / transaction)
