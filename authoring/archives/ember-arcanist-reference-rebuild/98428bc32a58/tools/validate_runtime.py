"""Read-only GLB audit for the explicitly authored Ember Arcanist runtime bundle.
...
def model_name(slot, lod):
    stem = "chr_civic_ember_arcanist_t1_m" if slot == "body" else "wep_civic_ember_arcanist_ember_staff" if slot == "weapon" else f"arm_civic_ember_arcanist_{slot}_t1_m"
            ...
            for lod, data in report["lods"].items():
                for slot, module in data["modules"].items():
                    ...
                    for record in records or []:
                        source = (runtime.parent / record["file"]).resolve()
                        if not source.is_relative_to(runtime.parent / "source") or not source.is_file():
