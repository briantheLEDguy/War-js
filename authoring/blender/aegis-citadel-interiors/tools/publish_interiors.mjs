...
for(const asset of report) for(const lod of asset.lods){
  ...
  for(const im of doc.images??[]){
    if(!im.uri?.startsWith('../textures/aegis_citadel_interiors/'))errors.push('Texture not shared externally');
  }
...
console.log(`${results.length} citadel furnishing GLBs; ${results.reduce((n,r)=>n+r.errors.length,0)} errors`);
...
if(process.argv.includes('--publish')){
  ...
  const preview='authoring/blender/aegis-citadel-interiors/review/all-exports.png';
  ...
  const textureOut=path.join(root,'public/assets/textures/aegis_citadel_interiors');
  await mkdir(textureOut,{recursive:true});
  for(const file of await readdir(path.join(work,'textures/aegis_citadel_interiors'))){
    await copyFile(path.join(work,'textures/aegis_citadel_interiors',file),path.join(textureOut,file));
  }
  ...
  for(const asset of report){
    ...
    Object.assign(bp,{assetId,displayName:`Aegis ${asset.kind.replaceAll('_',' ')}`,sets:['aegis_citadel_interiors']});
    bp.runtime={staticKey:`aegis_${asset.kind}`};
    bp.output={model:`${name}.glb`,artifactDir:'authoring/blender/aegis-citadel-interiors/runtime'};
    bp.generator.copyFrom=`authoring/blender/aegis-citadel-interiors/runtime/${name}.glb`;
    ...
    bp.materials={...bp.materials,master:'MM_AegisCitadelInteriors',textureSet:'aegis_citadel_interiors',
      channels:['baseColor','roughness','metallic','normal'],maxTextureResolution:2048};
    bp.provenance={...bp.provenance,source:'authoring/blender/aegis-citadel-interiors/tools/build_interiors.py',
      promptIds:['aegis_citadel_furnishing_v1'],referencePackId:'original_three_peoples_oath_and_fortress_household'};
  ...
  console.log(`Published ${report.length} reviewed citadel furnishings.`);
}
