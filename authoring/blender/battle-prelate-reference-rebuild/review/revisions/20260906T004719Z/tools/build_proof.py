...
def main():
    ...
    master_path=ROOT/("battle_prelate_reference_master.blend" if scene_data.get("stage")=="full_character" else "battle_prelate_upper_body_proof.blend")
    snapshots={p.name:p.read_bytes() for p in sorted((ROOT/"source").glob("*.json")) if p.name!="scene.json"}
    ...
    for obj in collection.all_objects:
        if obj.type=="EMPTY":
            (archive/(obj.name.removeprefix("component.")+".json")).write_text(obj["source_record"],encoding="utf-8")
    (archive/"scene.json").write_text(json.dumps(scene_data,indent=2))
    evidence={}
