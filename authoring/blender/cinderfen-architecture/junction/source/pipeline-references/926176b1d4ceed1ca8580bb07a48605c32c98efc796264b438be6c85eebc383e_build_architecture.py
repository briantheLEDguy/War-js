...
def prototype(name,lod,materials,collection):
    ...
    if lod<2 and record['bevel']>.001 and name not in ('fen_basalt','reed_roof_bundle','reed_wall_panel'):
...
def physical_uvs(data,scale,material_name):
    ...
    for polygon in data.polygons:
        ...
        result.append([(float(p[axes[0]]-lower[axes[0]]),float(p[axes[1]]-lower[axes[1]])) for p in vertices])
    return result
...
def combined(asset_id,lod,prototypes,materials,save_instances=False,rigid_group=None):
    ...
    for number,instance in enumerate(SOURCE['assets'][asset_id]['instances']):
        ...
        fitted_uvs=physical_uvs(data,instance['scale'],material_name)
