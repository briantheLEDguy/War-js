...
import { measuredStairRails } from './repair_stair_navigation.mjs';
...
const builder=await read(path.join(work,'builder-metadata.json'));
assert.equal(builder.runtimeReady,true,'package builder metadata accepted');
...
for(const asset of report){
  ...
  const placement=builder.assets[stem];assert.equal(placement.runtimeReady,true,stem+' builder publication');
  assert.equal(placement.modelSha256,asset.lods[0].sha256,stem+' builder bound to actual GLB');
  assert.deepEqual(placement.defaultScale,{x:1,y:1,z:1},stem+' real metre scale');
  assert.equal(placement.colliderSpace,'model',stem+' model-space navigation');
  assert.deepEqual(placement.walkableSurfaces,asset.contract.walkableSurfaces??[],stem+' measured supports');
  if(stem.endsWith('gate_leaves')){
    assert.deepEqual(placement.colliders,[{...asset.contract.closed_collision_runtime,blocksWhen:'closed'}]);
    assert.equal(placement.interaction.openClip,'gate_open');assert.equal(placement.interaction.closeClip,'gate_close');assert.equal(placement.interaction.startsOpen,false);
  }else {
    const added=stem==='frontier_cinderfen_wall_stair'?measuredStairRails(await read(path.join(work,'source/architecture.json'))).map(({sourcePiece,sourcePart,...box})=>box):[];
    assert.deepEqual(placement.colliders,[...asset.contract.colliders,...added],stem+' measured colliders');
  }
  ...
  for(const lod of qc.builtLods){
    ...
    for(const ref of refs){
      assert.match(ref.uri,/^\.\.\/textures\/cinderfen_architecture\/[a-f0-9]{64}\.png$/);
