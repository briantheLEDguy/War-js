...
if(!geometryOnly){
  ...
  assembly=await read(path.join(work,'review/assembly.json'));
  add(assembly.tool_sha256===await hashFile(path.join(work,'tools/review_assembly.py')),'Changed corner assembly review tool');
  add(assembly.preview_sha256===await hashFile(path.join(work,'review',assembly.preview)),'Changed corner assembly review image');
  for(const input of assembly.inputs)add(input.sha256===await hashFile(path.resolve(work,'..',input.model)),'Changed actual corner assembly model: '+input.model);
  add(assembly.clearance_hits.length===0&&assembly.stair_route_clearance_hits.length===0,'Corner assembly has an obstructed walking route');
  add(assembly.support_samples.every(sample=>sample.height!==null&&Math.abs(sample.height-6.3)<=.03),'Corner assembly has an unsupported walking route');
  add(source.reused_authored_construction.sha256===await hashFile(path.resolve(work,'../source/architecture.json')),'Corner source no longer matches the companion construction library');
}
for(const [assetId,definition] of Object.entries(source.assets)){
  ...
  for(const lod of built.lods){
    ...
    if(!geometryOnly){
      ...
      if(lod.level===0)lod.reviewImages[assembly.preview]=assembly.preview_sha256;
      ...
      for(const prefix of ['front','open','gameplay','side'])if(imported[prefix+'_preview']){
...
const makeBlueprint=asset=>({
  ...
  runtime:{staticKey:asset.asset_id},output:{model:asset.lods[0].model,artifactDir:'authoring/blender/cinderfen-architecture/junction/runtime'},
  generator:{kind:'copyExisting',copyFrom:'authoring/blender/cinderfen-architecture/junction/runtime/'+asset.lods[0].model},
  ...
  lifecycle:{status:'review_pending',notes:'Original pentagonal deck cages and fitted Cinderfen construction pieces remain editable in the Blender master. Closed construction pieces deliberately share metre-scaled tiled material UVs for reeds, basalt, alder and hardware; these are reusable material fields rather than a unique atlas. Positional audit welds glTF material/UV/normal seam splits at one micrometre and requires zero boundary and multi-face edges on every LOD. Coarse authored roof and deck courses preserve the curved envelope and support height while reducing distant seams. Exact bounds, raised floors, ramps, treads, colliders and keep passage contracts are retained in authoring/blender/cinderfen-architecture/junction/build-report.json. Internal visual acceptance is required before package publication.'},
});
