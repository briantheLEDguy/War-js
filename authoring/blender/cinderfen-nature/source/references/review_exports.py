...
def render(objects,path,front=False,gameplay_distance=None,side=False,focus=None,frame=None):
    ...
    if focus is not None:centre=Vector(focus);extent=frame
    ...
    if not gameplay_distance:
        ...
        data.ortho_scale=frame if frame else max(width,height*scene.render.resolution_x/scene.render.resolution_y)*1.14
    ...
    return {'camera_position_z_up':list(camera.location),'target_z_up':list(target),'projection':data.type,'lens_mm':data.lens if gameplay_distance else None,'distance_m':gameplay_distance,'detail_frame_m':frame,'lighting':'neutral white area lights; unmodified exported PBR channels','resolution':[scene.render.resolution_x,scene.render.resolution_y]}
...
def main(args):
    ...
    for path in sorted((ROOT/'runtime').glob('*.glb')):
        ...
        if args.resume and receipt.exists():
            previous=json.loads(receipt.read_text());views=[('preview','preview_sha256')]+[(prefix+'_preview',prefix+'_preview_sha256') for prefix in ('front','open','gameplay','side','material','reed') if prefix+'_preview' in previous]
        ...
        if path.stem=='frontier_cinderfen_workshop_lod0':
            for prefix,focus,frame in [('material',(-2.6,-4.08,2.7),3.0),('reed',(5.3,-1.8,4.58),2.8)]:
                detail=ROOT/'review'/f'{path.stem}_{prefix}.png';record[prefix+'_camera']=render(objects,detail,focus=focus,frame=frame)
                record[prefix+'_preview']=detail.name;record[prefix+'_preview_sha256']=sha(detail)
        if path.stem=='frontier_cinderfen_gatehouse_lod0':
            detail=ROOT/'review'/f'{path.stem}_material.png';record['material_camera']=render(objects,detail,focus=(-6.85,-6.2,2.2),frame=3.2)
            record['material_preview']=detail.name;record['material_preview_sha256']=sha(detail)
