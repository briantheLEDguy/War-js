...
const selectionArg=process.argv.find(argument=>argument.startsWith('--assets='));
if(process.argv.includes('--assets'))throw Error('Use --assets=key1,key2 for an explicit selection.');
const selection=publicationSelection(Object.keys(source.assets),selectionArg?.slice('--assets='.length));
const includesAlder=selection.ids.some(key=>key.includes('alder'));
...
for(const [key,definition] of Object.entries(source.assets)){
  if(!selection.ids.includes(key))continue;
...
await save(path.join(work,selection.validation),{passed:issues.length===0,completeReimportReview:!geometryOnly,issues,models});
await save(path.join(work,selection.build),assets);
...
if(publish){
  const reviewPath=path.join(work,selection.review),review=await read(reviewPath);
  const images=Object.assign({},...assets.flatMap(asset=>asset.lods.map(lod=>lod.reviewImages)));
  verifySelectionReview(review,selection,await hashFile(path.join(work,selection.build)),images);
  if(includesAlder&&review.alderJoinsSha256!==await hashFile(path.join(work,'review/alder-joins.json')))throw Error('Review must bind the actual alder join audit.');
  if(!selection.scoped)for(const name of ['all-exports.png','gameplay-exports.png'])if(review.contactSheets?.[name]!==await hashFile(path.join(work,'review',name)))throw Error('Unreviewed contact sheet: '+name);
  ...
  console.log('Published '+assets.length+' internally accepted ecology assets; global registry untouched.');
...
const buildHash=await hashFile(path.join(work,selection.build));
let previous={assets:{}};
try{previous=await read(path.join(work,'builder-metadata.json'));}catch(error){if(error.code!=='ENOENT')throw error;}
// Per-asset receipts let accepted siblings remain available while another model is revised.
const metadata={schemaVersion:1,units:'metres',upAxis:'+Y',frontAxis:'+Z',sourceSha256:sourceHash,
  buildSha256:selection.scoped?previous.buildSha256:buildHash,runtimeReady:false,assets:{...previous.assets}};
for(const asset of assets){
  const prior=previous.assets?.[asset.asset_id];
  const reviewFile=publish?selection.review:(prior?.reviewFile??'review/review.json');
  let runtimeReady=publish,reviewHash;
  if(publish)reviewHash=await hashFile(path.join(work,reviewFile));
  else if(!geometryOnly||prior?.runtimeReady){
    try{
      reviewHash=await hashFile(path.join(work,reviewFile));
      ...
      runtimeReady=prior?.runtimeReady===true&&approved.approvalState==='approved'&&approved.hashes.modelSha256===asset.lods[0].sha256&&approved.review.reviewHash===reviewHash;
      runtimeReady&&=approved.hashes.qcSha256===await hashFile(path.join(repo,'public/assets/models',approved.qc));
      for(const lod of asset.lods)runtimeReady&&=lod.sha256===await hashFile(path.join(repo,'public/assets/models',lod.model));
      const qc=await read(path.join(repo,'public/assets/models',approved.qc));
      runtimeReady&&=JSON.stringify(qc.placementContract)===JSON.stringify(asset.contract);
    }catch(error){if(error.code!=='ENOENT')throw error;runtimeReady=false;}
  }
  ...
  metadata.assets[asset.asset_id]={label:asset.displayName,group:'Cinderfen Nature',kind:asset.contract.kind,model:asset.lods[0].model,modelSha256:asset.lods[0].sha256,runtimeReady,
    ...(runtimeReady?{reviewFile,reviewHash,buildSha256:publish?buildHash:prior?.buildSha256}:{}),
    defaultScale:{x:1,y:1,z:1},colliderSpace:'model',footprint:{width:boundsYUp.maximum[0]-boundsYUp.minimum[0],depth:boundsYUp.maximum[2]-boundsYUp.minimum[2],chainAxis:'x'},boundsYUp,placementDatum:'Planted at model Y0; root and stone toes intentionally extend below ground.',colliders:asset.contract.colliders,walkableSurfaces:[],cameraSolid:asset.contract.cameraSolid};
}
metadata.runtimeReady=Object.keys(source.assets).every(key=>metadata.assets[key]?.runtimeReady===true);
