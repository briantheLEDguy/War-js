...
from greenskin_identity import sculpt,tusks
from peat_equipment import equip
from wet_workwear import finish as finish_wet_workwear
from peat_locomotion import fit_fall
...
def build(kind, recipe):
    if kind!='greenskin_peat_worker':raise ValueError('This package owns only the Cinderfen peat worker')
    ...
    if kind in ('dwarf_artisan', 'greenskin_peat_worker', 'empire_herbalist'):
        rows = [(mapped(z),w) for z,w in [(1.42,.13),(1.34,.16),(1.22,.19),(1.10,.20),(1.0,.20),(.94,.195),(.91,.205)]]
        verts = []
        ...
        for r, (z, width) in enumerate(rows):
            for c in range(13):
                ...
                hem_ease=.016*max(0,min(1,(mapped(1.01)-z)/.105))
                verts.append((x,y-hem_ease-.004*math.sin(t*math.tau*3)*(r/8),z))
    ...
    equip(make,sweep,{'leather':leather,'seam':seam,'wood':wood,'iron':iron},belt_z)
    ...
    if race=='greenskin':
        apron_surface=ClothSurface(apron)
        bind_detail(bpy.data.objects['work_belt'],[apron_surface],select=lambda point:point.y<-.06)
        bind_detail(bpy.data.objects['apron_bound_perimeter'],[apron_surface])
        supports=[apron_surface]+[ClothSurface(obj) for obj in authored if 'shirt_continuous_tailored_surface' in obj.name or 'standing_shirt_collar' in obj.name]
        bind_detail(bpy.data.objects['apron_continuous_neck_loop'],supports)
        sew_work_shirt(make,material,lambda point:morphology(point,race),rig)
        finish_wet_workwear(apron,make,sweep,{'leather':leather,'seam':seam},WORK)
    ...
    rig['skeletonId']='frontier_mire_worker_v1';rig['bindPoseId']='frontier_mire_worker_a_v1'
...
if __name__ == '__main__':
    selected = next((a.split('=',1)[1].split(',') for a in sys.argv if a.startswith('--assets=')), ['greenskin_peat_worker'])
