...
SOURCE = WORK / (f'runtime/frontier_cinderfen_greenskin_peat_worker_lod{lod}.glb' if lod is not None
                 else 'sources/frontier_cinderfen_greenskin_peat_worker.blend')
...
body = max((o for o in bpy.context.scene.objects if o.type == 'MESH' and not o.hide_render),key=lambda o:len(o.data.vertices)) if lod is not None else bpy.data.objects['greenskin_peat_worker_exposed_anatomy']
