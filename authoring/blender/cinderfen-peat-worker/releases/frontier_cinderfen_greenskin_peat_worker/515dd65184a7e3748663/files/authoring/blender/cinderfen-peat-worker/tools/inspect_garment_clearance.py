...
key=next((a.split('=',1)[1] for a in sys.argv if a.startswith('--asset=')),'frontier_cinderfen_greenskin_peat_worker')
...
rest=positions();belt=1.02*1.11
candidates=[tuple(face.vertices) for face in body.data.polygons if body.data.materials[face.material_index].name=='peat_worker_wax_worn_bib']
...
if band_extent[0]<.38 or band_extent[1]<.28:raise RuntimeError('Missing complete worker waist belt inspection surface: '+str(band_extent))
...
for side,sign in [('L',1),('R',-1)]:
    ...
    hem_probes[side]=np.array(sorted(i for i in trousers if .225<rest[i,2]<.275 and sign*rest[i,0]>.12),dtype=int)
...
for clip in doc['animations']:
    ...
    for seconds in sorted(times):
        ...
        posed=positions();indices,faces=apron_surface
