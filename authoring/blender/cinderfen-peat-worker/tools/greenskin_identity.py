...
def face_shape(point):
    ...
    xx=x*(1+mask*(.12+.29*jaw))
    yy=y-mask*(.038*jaw*face+.013*brow)+.007*nose
    zz=z-.032*smooth(1.96,2.046,z)
    ear=smooth(.073,.101,abs(x))*math.exp(-((z-1.91)/.072)**2)
    xx+=(1 if x>=0 else -1)*.029*ear
    zz+=.033*ear*smooth(1.89,1.93,z)
    ...
    for obj in meshes:
        for mat in obj.data.materials:
            ...
            if node.type=='TEX_IMAGE':
                image=node.image.copy();image.name='cinderfen_peat_worker_olive_skin'
                pixels=np.array(image.pixels[:],dtype=np.float32).reshape(-1,4)
                pixels[:,:3]*=np.array([.45,.67,.32])
...
def tusks(make,material):
    ivory=material('peat_worker_worn_ivory',(.49,.43,.28),.64,textile=True)
    for side in (-1,1):
        ...
        stations=[(.045,-.198,1.818,.0105,.0075),(.047,-.217,1.822,.011,.008),
                  ...
                  (.057,-.238,1.861,.0068,.0048),(.056,-.236,1.874,.0043,.003),
                  (.053,-.232,1.885,.0011,.001)]
        vertices=[];count=14
        centers=[Vector((side*x,y,z+(0.002 if side<0 else 0))) for x,y,z,_,_ in stations]
        ...
        tooth=make('peat_worker_curved_lower_tusk_'+str(side),vertices,faces,ivory,'jaw')
