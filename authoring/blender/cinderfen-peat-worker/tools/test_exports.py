...
KEY='frontier_cinderfen_greenskin_peat_worker'
...
class InhabitantExportTests(unittest.TestCase):
    def test_editable_outfit_is_continuous_closed_weighted_and_current(self):
        ...
        self.assertEqual(report['skeletonId'],'frontier_mire_worker_v1')
        self.assertEqual(report['bindPoseId'],'frontier_mire_worker_a_v1')
        self.assertEqual(len(report['objects']),9)
    ...
    def test_each_audit_measures_the_expected_imported_mesh(self):
        ...
        for row in build['lods']:
            lod=row['level']
            for filename in (f'arm-volume-lod{lod}.json',f'{KEY}_lod{lod}_motion.json',f'{KEY}_lod{lod}_garment_clearance.json',f'{KEY}_lod{lod}_welt.json'):
