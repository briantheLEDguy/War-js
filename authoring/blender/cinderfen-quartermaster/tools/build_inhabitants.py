...
from quartermaster_identity import sculpt,tusks
...
CAST = {
    ...
    'greenskin_quartermaster': {'foundation': 'mire_brutish_v1_m', 'race': 'greenskin', 'cloth': (.27, .18, .105), 'leather': (.09, .105, .12)},
...
def morphology(p, race):
    if race=='greenskin':
        x,y,z=p
        # Design-space dimensions are adapted to the retained racial anatomy.
        # The body and rig themselves are already sculpted and are not remorphed.
        return Vector((x*1.098,y*1.12,z*1.11))
...
def build(kind, recipe):
    if kind!='greenskin_quartermaster':raise ValueError('This package owns only the Cinderfen peat worker')
    ...
    waistcoat=make_waistcoat(make,sweep,material,{'leather':leather,'seam':seam,'brass':brass})
    ...
    belt_surfaces=[ClothSurface(o) for o in authored
                   if 'continuous_tailored_surface' in o.name or 'quartermaster_waistcoat' in o.name]
    ...
    if race=='greenskin':
        support=ClothSurface(waistcoat)
        bind_detail(bpy.data.objects['work_belt'],[support])
        for obj in authored:
            if obj.name.startswith('forged_buckle_frame'):bind_detail(obj,[ClothSurface(bpy.data.objects['work_belt'])])
        sew_work_shirt(make,material,lambda point:morphology(point,race),rig)
        finish_waistcoat(waistcoat,make,sweep,{'leather':leather,'seam':seam,'brass':brass})
        carrier_mount=equip(make,sweep,material,{'brass':brass,'seam':seam},belt_z)
    ...
    rig['skeletonId']='cinderfen_quartermaster_humanoid_v1';rig['bindPoseId']='cinderfen_quartermaster_a_v1'
...
if __name__ == '__main__':
    selected = next((a.split('=',1)[1].split(',') for a in sys.argv if a.startswith('--assets=')), ['greenskin_quartermaster'])
