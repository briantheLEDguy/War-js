...
SOURCE = WORK / (f'runtime/frontier_cinderfen_greenskin_quartermaster_lod{lod}.glb' if lod is not None
                 else 'sources/frontier_cinderfen_greenskin_quartermaster.blend')
...
body = max((o for o in bpy.context.scene.objects if o.type == 'MESH' and not o.hide_render),key=lambda o:len(o.data.vertices)) if lod is not None else bpy.data.objects['greenskin_quartermaster_exposed_anatomy']
...
if min(min(s['span']) for name,s in baseline.items() if name.startswith('forearm')) < .05:
    raise RuntimeError('Greenskin forearm has less than 5 cm thickness')
