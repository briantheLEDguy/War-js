"""Verify the closed waistcoat clears the shirt, trousers and exposed anatomy."""
...
tool=[tuple(f.vertices) for f in body.data.polygons if body.data.materials[f.material_index].name=='quartermaster_supply_waistcoat']
cloth_polygons=[f for f in body.data.polygons if any(t in body.data.materials[f.material_index].name for t in ('_woven_linen','_wool','body_mire_brutish_v1_m.body'))]
...
# Reject a broken rest garment before spending minutes on animation sampling.
p=rest[tool_ids];tree=BVHTree.FromPolygons(rest[cloth_ids],cloth_faces,all_triangles=True);rest_crossings=[]
for a,b in edges:
    start=Vector(p[a]);delta=Vector(p[b])-start
    if delta.length<.001:continue
    direction=delta.normalized();hit,_,triangle,_=tree.ray_cast(start+direction*.00025,direction,delta.length-.0005)
    if hit is not None:rest_crossings.append({'point':list(hit),'target':cloth_materials[triangle]})
if rest_crossings:
    failure={'model':source.name,'sha256':hashlib.sha256(raw).hexdigest(),'restCrossings':rest_crossings}
    (WORK/'review'/f'{KEY}_lod{lod}_layer_rest_rejection.json').write_text(json.dumps(failure,separators=(',',':')))
    raise RuntimeError('Waistcoat crosses underlying clothing/anatomy at rest: '+str(len(rest_crossings)))
...
report={'model':source.name,'sha256':hashlib.sha256(raw).hexdigest(),'importedTriangles':expected,'outerGarmentTriangles':len(tool),'clothTriangles':len(cloth),'collisionTargets':sorted(collision_targets),'restCrossings':len(rest_crossings),'clips':records}
(WORK/'review'/f'{KEY}_lod{lod}_layer_clearance.json').write_text(json.dumps(report,separators=(',',':')))
if any(row['maximumCrossings'] for row in records):raise RuntimeError('Layered waistcoat/tools cross animated clothing')
