...
def fit_fall(rig):
    ...
    body=bpy.data.objects['greenskin_quartermaster_exposed_anatomy'];skin=_surface(body)
    supports=[_surface(o) for o in bpy.context.scene.objects if o.type=='MESH' and
              ('continuous_tailored_surface' in o.name or 'quartermaster_waistcoat' in o.name or o.name=='work_belt' or o.name.startswith('forged_buckle'))]
    ...
    for frame in range(61):
        ...
        for side,sign in [('L',1),('R',-1)]:
            ...
            # Brace in the moving chest frame before reaching for the floor.
            # Blending a hanging hand directly to a world-ground target pulls
            # the chain straight through the broad torso during the early fall.
            chest=rig.pose.bones['upper_chest'];space=chest.matrix@chest.bone.matrix_local.inverted()
            anticipation=max(0,min(1,frame/25));anticipation=anticipation*anticipation*(3-2*anticipation)
            neutral=Vector((sign*.23,-.13,-.39));brace=Vector((sign*.23,-.36,.18))
            wrist=space@(upper.bone.head_local+neutral.lerp(brace,anticipation))
            reachable_height=max(target.z,wrist.z)
            wrist=wrist.lerp(target,t)
            # Groundward interpolation must not force an unreachable straight
            # arm while the chest is still high. Let the braced hand descend
            # with the body, then settle onto the same palm contact plane.
            wrist.z=reachable_height
            ...
            for _ in range(5):
                _solve_chain(rig,side,wrist,rotation,('upper_arm','forearm','hand'),Vector((sign,0,-.35)))
                low=float(_skin(skin,rig)[hands[side],2].min());correction=.005-low
                # Until the brace reaches the floor, contact fitting may lift
                # a penetrating hand but must not pull a raised one downward.
                grounded=reachable_height<=target.z+.0001
                wrist.z+=max(0,correction)+(min(0,correction) if grounded else 0)
        ...
        for correction_pass in range(4):
            ...
            for side,sign in [('L',1),('R',-1)]:
                ...
                wrist,rotation=targets[side]['hand'];_solve_chain(rig,side,wrist,rotation,('upper_arm','forearm','hand'),Vector((sign,0,-.35)))
