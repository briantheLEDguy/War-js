...
for view in views:
    ...
    if view=='gameplay':data.ortho_scale=height*4.4
    ...
    if sheet and clip!='death':
        ...
        data.ortho_scale=max(max(vertical)-min(vertical),(max(horizontal)-min(horizontal))*scene.render.resolution_y/scene.render.resolution_x)*1.15
    if 'boot' in view:
        foot=rig.pose.bones['foot_R'];target=rig.matrix_world@foot.head.lerp(foot.tail,.55)
        data.ortho_scale=.50;direction=Vector((.7,-1,.3))
...
(WORK/'review'/f'{key}_lod{lod}_{"motion_sheet" if sheet else "review"}{suffix}.json').write_text(json.dumps({'modelSha256':original_hash,'bounds':{'min':list(low),'max':list(high)},'images':records,'status':'pending'},separators=(',',':')))
