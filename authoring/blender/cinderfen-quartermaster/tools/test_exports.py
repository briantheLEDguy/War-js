...
class InhabitantExportTests(unittest.TestCase):
    def test_waistcoat_clears_linen_and_exposed_anatomy(self):
        for lod in range(3):
            report=json.loads((WORK/'review'/f'{KEY}_lod{lod}_layer_clearance.json').read_text())
            self.assertEqual(report['sha256'],sha(WORK/'runtime'/report['model']))
            self.assertEqual(len(report['clips']),9)
            self.assertGreater(report['outerGarmentTriangles'],100)
            self.assertEqual(report['restCrossings'],0)
            self.assertIn('body_mire_brutish_v1_m.body',report['collisionTargets'])
            self.assertTrue(any('_woven_linen' in name for name in report['collisionTargets']))
    ...
    def test_editable_outfit_is_continuous_closed_weighted_and_current(self):
        ...
        self.assertEqual(report['masterSha256'],sha(WORK/'sources'/f'{KEY}.blend'))
        self.assertEqual(report['skeletonId'],'cinderfen_quartermaster_humanoid_v1')
        self.assertEqual(report['bindPoseId'],'cinderfen_quartermaster_a_v1')
        self.assertEqual(len(report['objects']),13)
        ...
        for source in build['sourceFiles']+build['validationSourceFiles']:
            self.assertEqual(sha(WORK.parents[2]/source['path']),source['sha256'],source['path'])

    def test_sheathed_tool_clears_all_animated_clothing(self):
        for lod in range(3):
    ...
    def test_each_audit_measures_the_expected_imported_mesh(self):
        ...
        for row in build['lods']:
            lod=row['level']
            for filename in (f'arm-volume-lod{lod}.json',f'{KEY}_lod{lod}_motion.json',f'{KEY}_lod{lod}_garment_clearance.json',f'{KEY}_lod{lod}_welt.json',f'{KEY}_lod{lod}_equipment_attachment.json',f'{KEY}_lod{lod}_tool_clearance.json',f'{KEY}_lod{lod}_layer_clearance.json'):
    ...
    def test_waistcoat_clears_raised_knees(self):
        for lod in range(3):
    ...
    def test_belt_never_crosses_the_waistcoat(self):
        for lod in range(3):
            report=json.loads((WORK/'review'/f'{KEY}_lod{lod}_garment_clearance.json').read_text())
            self.assertEqual(report['sha256'],sha(WORK/'runtime'/report['model']))
