...
for (const filename of (await fs.readdir(path.join(root, 'review'))).filter(name => name.endsWith('_build.json'))) {
  ...
  for (const lod of build.lods) {
    ...
    await fs.writeFile(path.join(root, 'review', `${lod.model}.validation.json`), JSON.stringify(result) + '\n');
...
await fs.writeFile(path.join(root, 'review', 'draft-validation.json'), JSON.stringify(report) + '\n');
