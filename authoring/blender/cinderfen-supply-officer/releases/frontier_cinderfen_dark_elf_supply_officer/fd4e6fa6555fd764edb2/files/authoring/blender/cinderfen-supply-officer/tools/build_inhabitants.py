...
from officer_identity import sculpt, dress_hair
from officer_coat import extend_coat
from officer_equipment import equip,fit_equipment
from officer_locomotion import fit_fall, fit_gait_contacts, fit_run_counter_swing
...
CAST = {'dark_elf_supply_officer': {'foundation':'civic_humanoid_v2_f','race':'dark_elf','cloth':(.075,.09,.18),'leather':(.075,.055,.072)}}
...
def build_provenance(source,master,recipe,body):
    inputs={Path(__file__).resolve(),source.resolve(),master.resolve(),
            (FOUNDATIONS/f"{recipe['foundation']}.quad-surface.json.gz").resolve(),
            (FOUNDATIONS/f"{recipe['foundation']}.source.blend").resolve(),
            (FOUNDATIONS/f"body_{recipe['foundation']}.qc.json").resolve(),
...
def _body_morphology(p, race):
    x,y,z=p
    return Vector((x*.97,y*.96,z*1.045))


def morphology(p, race):
    # Garment patterns use the proven 1.86m design space; female anatomy uses
    # measured physical coordinates. Both finish in the same racial body space.
    x,y,z=p
    return _body_morphology((x/1.12,y/1.1,z/(1.86/1.76)),race)
...
def material(name, color, rough=.8, metal=0, textile=False):
    ...
    if textile:
        ...
        if 'directional_hair' in name:
            strand=np.sin(xx*.61+np.sin(yy*.014)*.6)+.3*np.sin(xx*1.77+yy*.003)
            relief=.08*strand; pigment=.87+.055*strand+.015*mottling
            roughness=np.clip(.72+.035*strand,.60,.82)
        elif 'tool_ash' in name:
            ...
            grain=np.sin(xx*.038+np.sin(yy*.013)*.6)+.23*np.sin(xx*.29)
            relief=.014*grain;pigment=.96+.022*grain+.015*mottling
            roughness=np.clip(rough+.025*grain,.5,.77)
...
def build(kind, recipe):
    if kind!='dark_elf_supply_officer':raise ValueError('This package owns only the Cinderfen supply officer')
    generating_inputs={Path(__file__).resolve()}
    for module in tuple(sys.modules.values()):
        filename=getattr(module,'__file__',None)
        if filename:
            path=Path(filename).resolve()
            if path.suffix=='.py' and path.is_relative_to(ROOT):generating_inputs.add(path)
    generating_inputs.update((FOUNDATIONS/f"body_{recipe['foundation']}.glb",FOUNDATIONS/f"{recipe['foundation']}.quad-surface.json.gz",ROOT/'scripts/blender-character-pipeline/data/body-families/canonical-animation-pack.json'))
    input_hashes={path:digest(path) for path in generating_inputs}
    bpy.ops.wm.read_factory_settings(use_empty=True)
    source = FOUNDATIONS / f"body_{recipe['foundation']}.glb"
    ...
    sculpt(meshes,rig,WORK)
    race = recipe['race']
    if race!='greenskin':
        for ob in meshes:
            for v in ob.data.vertices: v.co = _body_morphology(v.co, race)
    if race=='dwarf':
        body.data.update()
    ...
    if race!='greenskin':
        for bone in rig.data.edit_bones:
            bone.head = _body_morphology(bone.head, race); bone.tail = _body_morphology(bone.tail, race)
    ...
    extend_coat()
    dress_hair(body,make,sweep,material)
    if race == 'dwarf':
    ...
    bind_detail(bpy.data.objects['work_belt'],[support])
    belt_support=ClothSurface(bpy.data.objects['work_belt'])
    for obj in authored:
        if obj.name.startswith('forged_buckle_frame'):bind_detail(obj,[belt_support])
    carrier_mount=equip(make,sweep,material,{'leather':leather,'seam':seam,'wood':wood,'iron':iron,'brass':brass},belt_z)
    ...
    try:
        ...
        fit_gait_contacts(rig)
        fit_run_counter_swing(rig)
        fit_fall(rig)
        fit_equipment(make,rig,carrier_mount)
    finally:
        for modifier in muted:modifier.show_viewport=True
        bpy.context.view_layer.update()
    # The canonical contract is the input action set. The final head-rest and
    # fitted contact derivative has a distinct identity and embedded clips.
    rig['skeletonId']='cinderfen_supply_officer_humanoid_v1';rig['bindPoseId']='cinderfen_supply_officer_a_v1'
    ...
    for path,expected in input_hashes.items():
        if digest(path)!=expected:raise RuntimeError('Generating input changed during build: '+str(path))
...
if __name__ == '__main__':
    selected = next((a.split('=',1)[1].split(',') for a in sys.argv if a.startswith('--assets=')), ['dark_elf_supply_officer'])
