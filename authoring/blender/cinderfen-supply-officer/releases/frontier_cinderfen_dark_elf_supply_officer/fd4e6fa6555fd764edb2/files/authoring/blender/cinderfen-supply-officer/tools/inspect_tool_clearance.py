...
tool=[tuple(f.vertices) for f in body.data.polygons if any(t in body.data.materials[f.material_index].name for t in ('officer_ledger_case_','officer_inventory_metal','officer_case_suspension_'))]
cloth_polygons=[f for f in body.data.polygons if any(t in body.data.materials[f.material_index].name for t in ('_woven_linen','_wool','officer_split_coat','body_civic_humanoid_v2_f.body'))]
