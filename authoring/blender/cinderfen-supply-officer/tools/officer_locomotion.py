"""Fit the supply officer's fall to its actual body, boots, bib and hands."""
...
def fit_run_counter_swing(rig):
    """Carry the bent arm through a fuller sagittal swing without wrist IK."""
    action=bpy.data.actions['run'];poses=_sample_action(rig,action,20)
    rig.animation_data.action=action;rig.animation_data.action_slot=action.slots[0]
    for frame in range(21):
        bpy.context.scene.frame_set(frame);_restore_pose(rig,poses[frame])
        for side in ('L','R'):
            upper=rig.pose.bones['upper_arm_'+side]
            direction=rig.pose.bones['hand_'+side].head-upper.head
            # Rotate the whole bent chain; its length and elbow/wrist angles
            # remain unchanged while the shorter female stride reads clearly.
            angle=math.atan2(direction.y*1.12,-direction.z)-math.atan2(direction.y,-direction.z)
            rotation=Quaternion(Vector((1,0,0)),angle)@upper.matrix.to_quaternion()
            upper.matrix=Matrix.LocRotScale(upper.head.copy(),rotation,Vector((1,1,1)))
            bpy.context.view_layer.update()
            upper.keyframe_insert('rotation_quaternion',frame=frame,group=upper.name)
            upper.keyframe_insert('location',frame=frame,group=upper.name)
    for bag in _curves(action):
        for curve in bag.fcurves:
            if curve.data_path.startswith('pose.bones["upper_arm_'):
                for key in curve.keyframe_points:key.interpolation='LINEAR'
    rig.animation_data.action=None
    for bone in rig.pose.bones:bone.matrix_basis.identity()
    bpy.context.view_layer.update()

def fit_gait_contacts(rig):
    """Fit each boot's contact phase to the female limb chain, retaining flight."""
    ...
    for clip,duration in [('walk',30),('run',20)]:
        action=bpy.data.actions[clip];poses=_sample_action(rig,action,duration)
        rig.animation_data.action=action;rig.animation_data.action_slot=action.slots[0]
        minimum={side:100 for side in boots}
        for half_frame in range(duration*2+1):
            frame=half_frame/2;bpy.context.scene.frame_set(int(frame),subframe=frame%1);bpy.context.view_layer.update()
            for side,surface in boots.items():minimum[side]=min(minimum[side],float(_skin(surface,rig)[:,2].min()))
        lower={side:max(0,height-.005) for side,height in minimum.items()}
        for frame in range(duration+1):
            bpy.context.scene.frame_set(frame);_restore_pose(rig,poses[frame])
            targets={side:(rig.pose.bones['foot_'+side].head-Vector((0,0,lower[side])),rig.pose.bones['foot_'+side].matrix.to_quaternion()) for side in boots}
            hips=rig.pose.bones['hips'];matrix=hips.matrix.copy();matrix.translation.z-=max(lower.values())+.001;hips.matrix=matrix;bpy.context.view_layer.update()
            for side,(ankle,rotation) in targets.items():_solve_chain(rig,side,ankle,rotation,hint=Vector((0,-1,0)))
            hips.keyframe_insert('location',frame=frame,group='hips')
            for name in names:
                bone=rig.pose.bones[name];bone.keyframe_insert('rotation_quaternion',frame=frame,group=name);bone.keyframe_insert('location',frame=frame,group=name)
        for bag in _curves(action):
            for curve in bag.fcurves:
...
def fit_fall(rig):
    ...
    body=bpy.data.objects['dark_elf_supply_officer_exposed_anatomy'];skin=_surface(body)
    ...
    for frame in range(61):
        ...
        for side,sign in [('L',1),('R',-1)]:
            ...
            target=Vector((sign*.19,thigh.head.y+.83,.18))
            ...
            target=Vector((sign*.36,upper.head.y-.25,.075))
