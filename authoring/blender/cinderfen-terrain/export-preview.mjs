...
import { composeCinderfenEcology } from '../../../scripts/campaign/cinderfen-ecology.mjs';
const read = async file => JSON.parse(await fs.readFile(file, 'utf8'));
const contracts = loadCinderfenContracts({ staged: true });
const zone = composeCinderfenEcology(integrateCinderfen(composeCinderfenEnvironment(composeCinderfenLandscape(await read('public/assets/maps/cinderfen_outskirts.json')),
...
for (const prop of zone.props) {
  ...
  const qc = await read('public/assets/models/' + asset.qc);
  assets[key] = { lods: (qc.lods?.length ? qc.lods : [{ model: asset.model, sha256: asset.modelSha256 }])
    .map(lod => ({ url: '/assets/models/' + lod.model, sha256: lod.sha256 })) };
