...
const key = 'frontier_field_apothecary';
...
assert(build.sourceFiles.length > 80, 'Retain actual PBR sources and construction tools');
...
for (const lod of build.lods) {
  ...
  assert.equal(doc.materials.length, 9, 'Nine deliberate material sets');
  assert.equal(doc.meshes.length, 9, 'Nine material batches, no hidden construction meshes');
  ...
  for (const kind of ['sourceMaster', 'master']) {
    ...
    assert(master); assert.equal(master.sha256, lod[`${kind}Sha256`]); assert(master.packedImages >= 27);
  ...
  assert.equal(review.reviewerSha256, hash(await bytes('tools/review_station.py')));
...
assert.equal(master.feet.length, 4);
...
const result = { passed: true, key, buildSha256: hash(buildBytes), validationToolSha256: hash(await fs.readFile(fileURLToPath(import.meta.url))), records,
  colliders: contract.colliders.length, limitations: ['Static artwork; no preparation or harvesting interaction claimed.', 'Village host placement and full in-game navigation are verified by the integrating task.'] };
