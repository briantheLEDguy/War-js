/** Publish the universal preparation table after literal export and collision checks. */
...
const packagePath = 'authoring/blender/field-apothecary', key = 'frontier_field_apothecary';
const assetId = 'prop.frontier.field_apothecary', name = 'Field Apothecary Worktable';
const hash = bytes => createHash('sha256').update(bytes).digest('hex');
const jsonBytes = value => Buffer.from(JSON.stringify(value, null, 2) + '\n');

export async function publishFieldApothecary(publish = false) {
  ...
  for (const file of ['scripts/campaign/publish-field-apothecary.mjs', 'scripts/campaign/regional-publication-evidence.mjs',
  ...
  for (const lod of build.lods) {
    ...
    assert(!gltf.skins?.length && !gltf.animations?.length, 'Static table must not add a character rig');
    ...
    await local('tools/review_station.py', review.reviewerSha256);
  ...
  execFileSync(process.execPath, [inside(repo, `${packagePath}/tools/validate_station.mjs`), '--check'], { cwd: repo, stdio: 'pipe' });
  ...
  const provenance = { createdBy: 'original_authored_mesh_control_cages', aiAssisted: true,
    ...
    referencePackId: 'original_universal_field_apothecary_v1', similarityReview: 'not_required',
  ...
  const qcFiles = lods.map(lod => ({ file: lod.model.replace('.glb', '.qc.json'), bytes: jsonBytes({
    ...
    limitations: ['Static preparation furniture; existing station crafting remains separate from the model.'],
  }) }));
  ...
  const blueprint = { assetId, displayName: name, category: 'prop', version: '1.0.0', sets: ['universal_town_workshops'],
    ...
    materials: { master: 'MM_FieldApothecary', textureSet: key,
    ...
    qc: { allowNonManifold: false, allowUvOverlap: true, maxTris: lods[0].triangles,
      maxFileSizeMb: 20, maxDrawCalls: 9, maxMeshObjects: 9, requiresSkinnedMeshes: false, requiresPreview: true,
  ...
  for (const [relative, bytes] of writes) {
    ...
    await fs.writeFile(`${target}.apothecary-pending`, bytes); await fs.rename(`${target}.apothecary-pending`, target);
  }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(JSON.stringify(await publishFieldApothecary(process.argv.includes('--publish')), null, 2));
}
