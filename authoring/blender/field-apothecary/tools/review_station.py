...
from build_station import KEY,audit,sha,save
...
def render(objects,path,view,lod):
    ...
    target=Vector((0,0,.95));direction=Vector((2.7,-5,2.0));frame=2.9
    if view=='rear':direction=Vector((-2.7,5,1.5))
    if view=='detail':target=Vector((-.27,.13,1.29));direction=Vector((1.3,-3,1.2));frame=1.55
    if view=='joinery':target=Vector((-.79,-.2,.47));direction=Vector((-2,-4,1));frame=.94
    ...
    if view=='gameplay':camera.location=Vector((.6,-(4,12,28)[lod],1.72));data.type='PERSP';data.lens=38;scene.render.resolution_x=1280;scene.render.resolution_y=720
    ...
    return {'cameraZUp':list(camera.location),'targetZUp':list(target),'projection':data.type,'orthoScale':frame if data.type=='ORTHO' else None,'distanceM':(4,12,28)[lod] if view=='gameplay' else None,'lighting':'neutral white area lights, actual exported PBR, no retouching'}
...
if __name__=='__main__':
    ...
    for lod in map(int,args.lods.split(',')):
        ...
        print('APOTHECARY_REIMPORTED',lod,result['triangles'],flush=True)
