...
from build_rack import KEY,audit,sha,save
...
def render(objects,path,view,lod):
    ...
    target=Vector((0,0,1.15));direction=Vector((2.7,-5,1.4));frame=3.55
    if view=='rear':direction=Vector((-2.7,5,1.5))
    if view=='detail':target=Vector((-.12,.06,2.07));direction=Vector((1.0,-4,1.2));frame=1.02
    if view=='joinery':target=Vector((-.64,.035,.27));direction=Vector((-2,-4,1.1));frame=.72
    ...
    if view=='gameplay':camera.location=Vector((.6,-(6,16,34)[lod],1.72));data.type='PERSP';data.lens=38;scene.render.resolution_x=1280;scene.render.resolution_y=720
    ...
    return {'cameraZUp':list(camera.location),'targetZUp':list(target),'projection':data.type,'orthoScale':frame if data.type=='ORTHO' else None,'distanceM':(6,16,34)[lod] if view=='gameplay' else None,'lighting':'neutral white area lights, actual exported PBR, no retouching'}
...
if __name__=='__main__':
    ...
    for lod in map(int,args.lods.split(',')):
        ...
        print('RACK_REIMPORTED',lod,result['triangles'],flush=True)
