...
def fields(name):
    ...
    if name=='oak':
        ...
        color=np.array([.29,.32,.34])[None,None,:]*(.75+broad[:,:,None]*.35)
        color+=oxide[:,:,None]*np.array([.20,.055,.008])
        height=medium*.045+fine*.035;rough=.30+broad*.17;metal=.93
    elif name=='steel':
        color=np.array([.63,.66,.67])[None,None,:]*(.94+medium[:,:,None]*.06)
        height=medium*.002+fine*.0008;rough=.27+medium*.08;metal=.96
    elif name=='brass':
