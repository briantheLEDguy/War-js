...
const key = 'frontier_field_command_table';
...
for (const record of construction.records) {
  ...
  assert(record.contacts.length >= 80 && record.contacts.every(contact => contact.passed));
  assert(record.actualImportSurface.passed && record.actualImportSurface.maximumVertexDeviationM <= .000002);
  await checked(record.finishedParts, record.finishedPartsSha256);
}
...
for (const lod of build.lods) {
  ...
  assert.equal(doc.materials.length, 6, 'Six deliberate material sets');
  assert.equal(doc.meshes.length, 6, 'Six material batches, no hidden construction meshes');
  assert(!doc.animations?.length && !doc.skins?.length, 'Static workshop asset');
  assert(doc.images.length >= 18 && doc.images.every(image => image.uri === undefined && Number.isInteger(image.bufferView)), 'Embedded runtime PBR images');
  ...
  for (const kind of ['sourceMaster', 'master']) {
    ...
    assert(master); assert.equal(master.sha256, lod[`${kind}Sha256`]); assert(master.packedImages >= 18);
  ...
  assert.equal(review.reviewerSha256, hash(await bytes('tools/review_table.py')));
...
const finishedParts = JSON.parse(await checked(build.lods[0].finishedParts, build.lods[0].finishedPartsSha256));
const ownedParts = contract.collisionMeasurements.flatMap(mass => mass.parts);
assert.equal(new Set(ownedParts).size, ownedParts.length, 'Collision mass ownership must be unique');
assert.deepEqual([...ownedParts].sort(), Object.keys(finishedParts).sort(), 'Every visible component belongs to a named collision mass');
...
for (const [index, collider] of contract.colliders.entries()) {
  ...
  const points = contract.collisionMeasurements[index].parts.flatMap(name => finishedParts[name].vertices);
  const actual = { minimum: [0, 1, 2].map(axis => Math.min(...points.map(p => p[axis]))),
    maximum: [0, 1, 2].map(axis => Math.max(...points.map(p => p[axis]))) };
  assert.deepEqual(measured, actual, 'Collision bounds must measure current finished component surfaces');
  ...
  for (let z = contract.standingPoint.z; z < 2.5; z += .05) {
...
const result = { passed: true, key, buildSha256: hash(buildBytes), validationToolSha256: hash(await fs.readFile(fileURLToPath(import.meta.url))), records,
  colliders: contract.colliders.length, limitations: ['Static campaign chart and tools; no map interaction, pickup or tabletop walking claimed.', 'Placement and gameplay acceptance are outside this authoring gate.'] };
