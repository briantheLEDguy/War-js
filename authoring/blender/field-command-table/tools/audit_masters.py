...
from build_table import KEY,audit,sha,save,bounds
...
for lod in(0,1,2):
    ...
    for kind in('sourceMaster','master'):
        ...
        if kind=='sourceMaster' and lod==0:
            ...
            footnames=[f'shaped_ground_skid_{x}' for x in(-1,1)]
...
save(ROOT/'review/master-audit.json',report);print('COMMAND_TABLE_MASTERS_VERIFIED',len(records),flush=True)
