...
run(blender('build_table.py'), 'build.log')
run(blender('audit_masters.py'), 'masters.log')
run(blender('inspect_construction.py'), 'contacts.log')
run(blender('review_table.py', '--lods', '0', '--views', 'neutral,rear,detail,joinery,gameplay'), 'views.log')
run(blender('review_table.py', '--lods', '1,2', '--views', 'neutral,gameplay'), 'lod-views.log')
run([sys.executable, str(ROOT / 'tools/consolidate_build.py')], 'consolidate.log')
run(['node', str(ROOT / 'tools/validate_table.mjs')], 'validate.log')
