...
const key = 'frontier_field_supply_chest';
...
for (const lod of build.lods) {
  ...
  assert.equal(doc.materials.length, 4, 'Four deliberate material sets');
  assert.equal(doc.meshes.length, 4, 'Four material batches, no hidden construction meshes');
  assert(!doc.animations?.length && !doc.skins?.length, 'Static workshop asset');
  assert(doc.images.length >= 12 && doc.images.every(image => image.uri === undefined && Number.isInteger(image.bufferView)), 'Embedded runtime PBR images');
  ...
  for (const kind of ['sourceMaster', 'master']) {
    ...
    assert(master); assert.equal(master.sha256, lod[`${kind}Sha256`]); assert(master.packedImages >= 12);
  ...
  assert.equal(review.reviewerSha256, hash(await bytes('tools/review_chest.py')));
...
const result = { passed: true, key, buildSha256: hash(buildBytes), validationToolSha256: hash(await fs.readFile(fileURLToPath(import.meta.url))), records,
  colliders: contract.colliders.length, limitations: ['Static closed chest; no loot or opening interaction claimed.', 'Placement and gameplay acceptance are outside this authoring gate.'] };
