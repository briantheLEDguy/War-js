...
from build_chest import KEY,audit,sha,save
...
def render(objects,path,view,lod):
    ...
    target=Vector((0,0,.39));direction=Vector((2.7,-5,2.0));frame=1.95
    if view=='rear':direction=Vector((-2.7,5,1.5))
    if view=='detail':target=Vector((0,-.24,.56));direction=Vector((1.3,-3,1.2));frame=.98
    if view=='joinery':target=Vector((-.615,-.29,.365));direction=Vector((-2,-4,1));frame=.61
...
if __name__=='__main__':
    ...
    for lod in map(int,args.lods.split(',')):
        ...
        print('CHEST_REIMPORTED',lod,result['triangles'],flush=True)
