...
before=next(row for row in json.loads((WORK/'review/welt-before-k-animation.json').read_text()) if row['lod']==lod)
report={'model':source.name,'sha256':hashlib.sha256(raw).hexdigest(),'importedTriangles':imported_triangles,
        ...
        'animationChannelSha256':fingerprints,'unchangedAnimationChannels':fingerprints==before['animationChannelSha256'],'clips':records}
...
if not report['unchangedAnimationChannels']:raise RuntimeError('Welt-only revision changed accepted animation channels')
