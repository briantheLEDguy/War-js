...
def fit_prone_contact(rig):
    ...
    for frame in range(61):
        ...
        poses.append({name:rig.pose.bones[name].matrix.copy() for name in names})
    for bag in _curves(action):
        for curve in list(bag.fcurves):
            if any(curve.data_path.startswith('pose.bones["'+name+'"]') for name in names):bag.fcurves.remove(curve)
    for frame in range(61):
        bpy.context.scene.frame_set(frame)
        for name in names:
            rig.pose.bones[name].matrix=poses[frame][name];bpy.context.view_layer.update()
        ...
        for side,sign in [('L',1),('R',-1)]:
            ...
            target=Vector((sign*.21,hips.y+length*.985,.14))
            ankle=foot.head.lerp(target,t)
            rotation=foot.matrix.to_quaternion().slerp(Quaternion(Vector((0,0,1)),math.pi)@rig.data.bones['foot_'+side].matrix_local.to_quaternion(),t)
