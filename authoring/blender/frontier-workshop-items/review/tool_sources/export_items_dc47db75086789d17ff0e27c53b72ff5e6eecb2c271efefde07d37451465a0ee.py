...
def repair_grazing_corner_normals(data,invalid):
    # On a tight smooth rope bend an averaged normal can become almost tangent
    # to its own face, collapsing Mikk's projected basis. Move only that corner
    # by under one degree toward its face; retain the authored surface exactly.
    normals=[entry.vector.copy() for entry in data.corner_normals];repairs=[]
    for face in data.polygons:
        for index in face.loop_indices:
            if index not in invalid:continue
            alignment=normals[index].dot(face.normal)
            if not face.use_smooth or face.area<1e-10 or not 0<alignment<.01:continue
            before=normals[index].copy();normals[index]=(before+face.normal*.01).normalized()
            repairs.append({'loop':index,'face':face.index,'normalBefore':list(before),'normalAfter':list(normals[index]),
                'faceAlignmentBefore':alignment,'faceAlignmentAfter':normals[index].dot(face.normal),
                'changeDegrees':math.degrees(before.angle(normals[index]))})
    if repairs:data.normals_split_custom_set(normals);data.update()
    return repairs
...
def bake(obj,name,lod,fields):
    ...
    normal_repairs=repair_grazing_corner_normals(data,invalid) if invalid else []
    if normal_repairs:
        data.calc_tangents(uvmap='runtime_uv');invalid=[l.index for l in data.loops if l.tangent.length<.99];data.free_tangents()
    uv=data.uv_layers['runtime_uv'].data;occupied=0
    for face in data.polygons:
    ...
    return {'group':name,'geometrySha256Before':before,'geometrySha256After':after,'atlas':{'resolution':resolution,'occupiedFraction':occupied,'marginPixels':4,'dilationPixels':2,'uvChartRepairs':chart_repairs,'cornerNormalRepairs':normal_repairs},'channels':channels}
