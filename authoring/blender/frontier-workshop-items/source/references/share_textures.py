...
def pack(file):
    ...
    for image in doc.get('images',[]):
        if 'bufferView' in image:
            ...
            filename=sha(pixels)+'.png';target=ROOT/'textures/cinderfen_nature'/filename;target.write_bytes(pixels)
            image['uri']='../textures/cinderfen_nature/'+filename
...
if __name__=='__main__':
    (ROOT/'textures/cinderfen_nature').mkdir(exist_ok=True)
