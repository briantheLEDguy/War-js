...
def render(objects,path,distance=None,detail=False,detail_target=None,detail_frame=None,camera_direction=None):
    ...
    if detail_target is not None:centre=Vector(detail_target);frame=detail_frame or 1.0
    if distance:
        ...
        camera.location=centre+(Vector(camera_direction)*extent if camera_direction else Vector((extent*.8,-extent*1.6,extent*.38)));target=centre;data.type='ORTHO'
