...
def pack(file):
    ...
    for image in doc.get('images',[]):
        if 'bufferView' in image:
            ...
            filename=sha(pixels)+'.png';target=ROOT/'textures/frontier_workshop_items'/filename;target.write_bytes(pixels)
            image['uri']='../textures/frontier_workshop_items/'+filename
...
if __name__=='__main__':
    (ROOT/'textures/frontier_workshop_items').mkdir(exist_ok=True)
    ...
    print('Shared textures:',len(list((ROOT/'textures/frontier_workshop_items').glob('*.png'))))
