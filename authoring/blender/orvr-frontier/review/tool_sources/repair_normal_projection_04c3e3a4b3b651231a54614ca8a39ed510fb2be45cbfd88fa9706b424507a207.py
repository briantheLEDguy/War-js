...
def main():
    ...
    retained=json.loads(output.read_text())['records'] if output.exists() else []
    ...
    for asset_id in args.assets.split(','):
        ...
        for level in (0,1,2):
            record=repair(asset_id,level,build); records[(asset_id,level)]=record
    ...
    output.write_text(json.dumps({'records':[records[key] for key in sorted(records)],'visual_approval':False},indent=2)+'\n')
