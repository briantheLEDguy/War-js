"""Retain original cages and placed masters; export finished architectural LODs."""
import argparse
import hashlib
import json
import math
...
def material(name):
    item=SOURCE['materials'][name];result=bpy.data.materials.new('sunmeadow.'+name);result.use_nodes=True
...
def prototype(name,lod,materials,collection):
    base=SOURCE['parts'][name];record=base.get('lod2',base) if lod==2 else base
    ...
    data.materials.append(materials[record['material']]);obj=bpy.data.objects.new(f'cage.{name}',data);collection.objects.link(obj)
    obj['description']=record['description'];obj['source']='literal_authored_mesh';obj['visible_geometry_source']='source/architecture.json'
    if lod<2 and record['bevel']>.001 and name not in ('coursed_ashlar','mortar_bed','slate_course'):
    ...
    protected=('arch_spandrel','arch_voussoir','splayed_arrow_slit','gate_leaf_plank','oak_joined_post','ledged_plank','square_nail')
    if lod>0 and name not in protected and not (lod==2 and 'lod2' in base):
...
def combined(asset_id,lod,prototypes,materials,save_instances=False,rigid_group=None):
    ...
    for number,instance in enumerate(SOURCE['assets'][asset_id]['instances']):
        ...
        if lod==2 and instance['part']=='mortar_bed':continue
        ...
        if material_name not in used_names:used_names.append(material_name)
        material_index=used_names.index(material_name)
        for polygon in data.polygons:
            faces.append(tuple(offset+index for index in polygon.vertices))
            coords=[tuple(data.uv_layers.active.data[index].uv) for index in polygon.loop_indices]
            corners.append(coords);face_materials.append(material_index)
        if save_instances:
            obj=bpy.data.objects.new(f'{name}.{number:04}',data);collection.objects.link(obj);obj.matrix_world=transform;obj['authored_part']=name
            obj.material_slots[0].link='OBJECT';obj.material_slots[0].material=materials[material_name]
...
def render(objects,path,front=False):
    ...
    data.ortho_scale=max(width,height*1800/1300)*1.16
...
def build_asset(asset_id,args):
    ...
    for lod in args.lods:
        ...
        animated=asset_id=='frontier_sunmeadow_gate_leaves'
