...
def pack(file):
    ...
    for image in doc.get('images',[]):
        if 'bufferView' in image:
            ...
            filename=sha(pixels)+'.png';target=ROOT/'textures/sunmeadow_architecture'/filename;target.write_bytes(pixels)
            image['uri']='../textures/sunmeadow_architecture/'+filename
...
if __name__=='__main__':
    (ROOT/'textures/sunmeadow_architecture').mkdir(exist_ok=True)
    ...
    print('Shared textures:',len(list((ROOT/'textures/sunmeadow_architecture').glob('*.png'))))
