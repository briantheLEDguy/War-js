...
SOURCE = WORK / (f'runtime/frontier_sunmeadow_empire_farmer_lod{lod}.glb' if lod is not None
                 else 'sources/frontier_sunmeadow_empire_farmer.blend')
...
body = max((o for o in bpy.context.scene.objects if o.type == 'MESH' and not o.hide_render),key=lambda o:len(o.data.vertices)) if lod is not None else bpy.data.objects['empire_farmer_exposed_anatomy']
...
if min(min(s['span']) for name,s in baseline.items() if name.startswith('forearm')) < .038:
    raise RuntimeError('Human forearm has less than 3.8 cm thickness')
