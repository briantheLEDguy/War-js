...
def fit_prone_contact(rig):
    ...
    skin_surface=_surface(bpy.data.objects['empire_farmer_exposed_anatomy'])
    ...
    trouser_surface=_surface(bpy.data.objects['trousers_continuous_tailored_surface'])
    knee_indices={side:[i for i,p in enumerate(trouser_surface[0]) if .4<p[2]<.75 and p[0]*sign>.08] for side,sign in [('L',1),('R',-1)]}
    hand_indices={}
    for side in ('L','R'):
        membership={}
...
def refine_sole_contacts(rig):
    ...
    knees={side:[i for i,p in enumerate(trousers[0]) if .4<p[2]<.75 and p[0]*sign>.08] for side,sign in [('L',1),('R',-1)]}
