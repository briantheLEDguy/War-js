...
from farmer_details import dress_farmer
...
CAST = {
    ...
    'empire_farmer': {'foundation': 'civic_humanoid_v2_m', 'race': 'empire', 'cloth': (.37, .285, .155), 'leather': (.19, .09, .043)},
...
def build(kind, recipe):
    ...
    if kind=='empire_farmer':
        sew_work_shirt(make,material,lambda point:morphology(point,race),rig)
        dress_farmer(make,sweep,material,rig,{'cloth':cloth,'leather':leather,'seam':seam,'iron':iron,'brass':brass,'wood':wood})
    ...
    if race in ('dwarf', 'empire'):
        ...
        try:
            ...
            bpy.context.view_layer.update()
    rig['skeletonId'] = 'sunmeadow_farmer_humanoid_v1'; rig['bindPoseId'] = 'sunmeadow_farmer_a_pose_v1'
...
if __name__ == '__main__':
    selected = next((a.split('=',1)[1].split(',') for a in sys.argv if a.startswith('--assets=')), ['empire_farmer'])
