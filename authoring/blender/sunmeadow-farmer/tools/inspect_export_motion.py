...
key=next((a.split('=',1)[1] for a in sys.argv if a.startswith('--asset=')),'frontier_sunmeadow_empire_farmer')
...
torso_indices=np.array(sorted({i for face in body.data.polygons if '_woven_linen' in body.data.materials[face.material_index].name for i in face.vertices if 1.12<rest[i,2]<1.45 and abs(rest[i,0])<.19} | {i for face in body.data.polygons if '_handled_brass' in body.data.materials[face.material_index].name for i in face.vertices}),dtype=int)
