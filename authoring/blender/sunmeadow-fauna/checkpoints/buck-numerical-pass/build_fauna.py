...
def paint_material(kind,definition):
    ...
    tile_x=(xx//512).astype(int);tile_y=(yy//512).astype(int);tile=tile_y*4+tile_x
    u=(xx%512)/511;v=(yy%512)/511
    ...
    # Broken interleaved fur strokes, with irregular tips and directional grain.
    cell_u=np.floor(u*77);cell_v=np.floor(v*43)
    seed=np.mod(np.sin(cell_u*127.1+cell_v*311.7)*43758.5453,1)
    local_u=np.mod(u*77,1);local_v=np.mod(v*43,1)
    shaft=np.exp(-((local_u-(.23+.54*seed)-.12*(local_v-.5))/.075)**2)
    breaks=np.sin(np.pi*local_v)**4*(.35+.65*seed)
    mottling=.013*np.sin(u*41+v*17)*np.cos(v*29-u*11)
    pigment=.027*shaft*breaks+mottling
    color=np.clip(color+pigment[...,None],0,1)
    ...
    surface=.5+shaft*breaks*.010
    surface=np.where(tile==2,.5+shaft*.008,surface)
    surface=np.where(tile==4,.5,surface)+antler_grain*.25
    dy,dx=np.gradient(surface);normal=np.stack([-dx*18,-dy*18,np.ones_like(dx)],axis=-1);normal/=np.linalg.norm(normal,axis=-1)[...,None]
    rough=np.where(tile==4,.17,np.where(tile==2,.36,.82))
...
def ear_patch(cage,definition,side,lod):
    points=[Vector((p[0]*side,p[1],p[2])) for p in definition['ear']];width=definition['ear_width'];rows=[0,.15,.38,.62,.83,1] if lod<2 else [0,.38,.72,1]
    ...
    for back in [False,True]:
        for t in rows:
            ...
            for across in [-1,0,1]:
                vertices.append(center+Vector((span*across,.017*math.sin(t*math.pi)*(1-across*across)+(.008 if back else 0),0)))
                coords.append(atlas((across+1)/2,t,0 if back else 3));weights.append({bone:1})
    faces=[];uv=[];count=len(rows)*3
    for back in [0,1]:
        for row in range(len(rows)-1):
            for column in range(2):
                a=back*count+row*3+column;face=[a,a+1,a+4,a+3];face=face[::-1] if back else face;faces.append(face);uv.append([coords[i] for i in face])
    for row in range(len(rows)-1):
        for column in [0,2]:
            a=row*3+column;face=[a,a+3,a+3+count,a+count];faces.append(face)
...
def unify_skin(cage,key,material,lod,definition):
    ...
    skin=cage.subset(is_skin);extras=cage.subset(lambda name:not is_skin(name));obj=skin.object(key+'_continuous_skin',material)
    ...
    for part in extras.parts:
        ...
        for local,index in enumerate(range(part['first_vertex'],part['first_vertex']+part['vertices'])):
            ...
            if hit is not None:extras.vertices[index][0]=hit.x+side*(.0003+depths[local//segments]*definition['eye_size']/.021)
...
def build(kind,definition,rest_only=True):
    ...
    (ROOT/'review'/f'{key}_build.json').write_text(json.dumps({'asset':key,'status':'anatomy-prototype-motion-pending' if rest_only else 'anatomy-and-motion-review-required','source_sha256':sha(ROOT/'source/anatomy.json'),'builder_sha256':sha(__file__),'rig_helper_sha256':sha(ROOT/'tools/quadruped_rig.py'),'motion':motion,'master':str(master.relative_to(ROOT)),'master_sha256':sha(master),'lods':lods,'bones':bones},indent=2,default=list)+'\n')
