...
def solve_leg(rig,limb,label,foot):
    ...
    if scapula:
        ...
        for step in range(-35,36):
            angle=step*.018;shoulder=origin+Quaternion(Vector((1,0,0)),angle)@rest_direction
            ...
            candidates.append((score,shoulder,elbow,reached,error))
        _,start,joint,ankle,error=min(candidates,key=lambda value:value[0])
...
def mammal_clips(rig,kind,definition):
    scale=definition.get('scale',1);fps=30;clips=[];reports=[]
    ...
    for clip in definition['clips']:
        ...
        for frame in range(last+1):
            ...
            pelvis=rig.pose.bones['pelvis'];bob=(.006*math.cos(time*2) if clip=='walk' else -.028+.020*math.cos(time*2) if running else .0015*math.sin(time))*scale
            ...
            for side,label in [(1,'L'),(-1,'R')]:
                ...
                for limb in ['front','hind']:
                    ...
                    if moving:
                        p=(phase+(gallop if running else walking)[limb+'_'+label])%1;stance=.40 if running else .64
                        stride=(.48 if running else .27)*scale
                        if p<stance:foot.y+=stride*(p/stance-.5)
                        else:
                            swing=(p-stance)/(1-stance);foot.y+=stride*(.5-smooth(swing));foot.z+=math.sin(math.pi*swing)*(.15 if running else .065)*scale
        ...
        reports.append({'clip':clip,'duration':last/fps,'frames':last+1,'maximum_ik_reach_error_m':maximum_error,'loop':clip not in ['attack','hit','death'],'motion_space':'in_place','status':'deformation_review_required'})
    reset(rig)
