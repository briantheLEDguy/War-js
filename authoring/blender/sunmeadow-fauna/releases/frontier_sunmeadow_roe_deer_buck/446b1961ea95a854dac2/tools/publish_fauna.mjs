...
for (const kind of selected) {
  ...
  if (candidateDirectory) {
    const delivery = await collectCandidateDelivery(root, key, resolve(kind).clips, candidateDirectory, { standingApproval });
...
if (process.argv.includes('--prepare-review') || standingApproval) {
  let previous = { assets: {} };
  ...
  for (const [kind, fresh] of evidence) {
    ...
    previous.assets[key] = standingApproval ? standingWorldAssetApproval(fresh) : { evidence: fresh, status: same ? old.status : 'pending', reviewedBy: same ? old.reviewedBy : '',
