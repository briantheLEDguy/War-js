...
def inspect(kind,lod,candidate_dir=None):
    delivery=ROOT/candidate_dir if candidate_dir else ROOT/'runtime'
    key='frontier_sunmeadow_'+kind;model=delivery/f'{key}_lod{lod}.glb';model_sha=sha(model)
    ...
    rig.animation_data_create();activate_imported_clip(rig,[mesh],'rest')
    ...
    clips=[]
    for clip_name in exported_times:
        action=activate_imported_clip(rig,[mesh],clip_name);first,last=action.frame_range
        key_times=exported_times[clip_name];samples=inspection_times(key_times)
...
if __name__=='__main__':
    ...
    for kind in args.assets.split(','):
        for lod in map(int,args.lods.split(',')):inspect(kind,lod,args.candidate_dir)
