"""Source-only proper local surface-frame study on the current shoulder."""
import math
import argparse
import sys
...
from joint_correctives import JointRelaxation, fascia_displacement, haunch_detail_retention, smoothstep
from reimport_review import setup

scale = resolve('roe_deer_buck')['scale']
parser = argparse.ArgumentParser(); parser.add_argument('--front-strength', type=float, default=1); parser.add_argument('--surface-frame', action='store_true'); parser.add_argument('--front-blend', type=float, default=1)
...
for phase in [.125, 7/9]:
    ...
    rest = np.asarray([v.co[:] for v in obj.data.vertices]); edges = np.asarray([e.vertices[:] for e in obj.data.edges])
    baseline = JointRelaxation(rest, edges, scale)
    relax = JointRelaxation(rest, edges, scale, front_strength=args.front_strength)
    obj.shape_key_clear(); action = activate_imported_clip(rig, [obj], 'run')
    ...
    positions = np.asarray([v.co[:] for v in posed.vertices]); rotations = np.zeros((len(rest), 3, 3))
    ...
    fascia = fascia_displacement(rest, scale, carrier, angles); retained = haunch_detail_retention(rest, scale, angles)
    old = positions+np.einsum('nij,nj->ni', rotations, baseline.delta(positions, rotations, fascia, retained))
    adjusted = positions+np.einsum('nij,nj->ni', rotations, relax.delta(positions, rotations, fascia, retained, surface_front=args.surface_frame))
    blend = args.front_blend+(1-args.front_blend)*smoothstep(-.09, 0, rest[:, 1]/scale)
    adjusted = positions+(adjusted-positions)*blend[:, None]
    ...
    print('SHOULDER_DIFFUSION_STUDY', phase, args.front_strength, args.surface_frame, args.front_blend, 'raw/before/after', strain(positions), strain(old), strain(adjusted), flush=True)
    mesh = bpy.data.meshes.new('surface_frame_study')
    ...
    label = 'surface' if args.surface_frame else 'bone'
    output = ROOT/'review/probes'/f'shoulder_diffusion_{args.front_strength:g}_{label}_blend{args.front_blend:g}_{phase:.3f}.png'
    _, camera = setup([obj], output, 1000, 20); center = Vector((0, -.24, .47)); camera.location = center+Vector((-3, -2.1, .3))
