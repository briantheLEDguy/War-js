...
def painted(name, folder, tint=None):
    mat = bpy.data.materials.new('captain_authored_' + name)
    mat.use_nodes = True
    ...
    for channel,socket in [('basecolor','Base Color'),('roughness','Roughness')]:
        ...
        if tint and channel=='basecolor':
            ...
            img=img.copy(); img.name='captain_chestnut_hair_albedo'
    ...
    if height_path.exists():
        ...
        image=bpy.data.images.new('captain_'+name+'_normal',width=width,height=height,alpha=True)
...
def build_head(make, morph, race):
    ...
    for path in [PRELATE/'source/head.json',ARCANIST/'source/hair.json']:
        ...
        for part in document['parts']:
            ...
            obj=make('captain_'+part['id'],vertices,faces,materials[names[0]],'head')
    ...
    head=bpy.data.objects['captain_head_skin']
    tree=BVHTree.FromObject(head,bpy.context.evaluated_depsgraph_get())
    scalp=bpy.data.objects['captain_auburn_scalp']
...
def build_swept_hair(make,shape,hair,tree):
    ...
    for index,path in enumerate(paths):
        ...
        ob=make('captain_volumetric_swept_lock_'+str(index),vertices,faces,hair,'head')
...
def hair_material():
    ...
    normal/=np.linalg.norm(normal,axis=-1,keepdims=True)
    mat=bpy.data.materials.new('captain_chestnut_directional_hair');mat.use_nodes=True
    ...
    for channel,values in [('basecolor',color),('normal',normal*.5+.5),('roughness',np.repeat(roughness[:,:,None],3,axis=2))]:
        ...
        image=bpy.data.images.new('captain_hair_'+channel,width=size,height=size,alpha=True)
        image.colorspace_settings.name='sRGB' if channel=='basecolor' else 'Non-Color'
        image.pixels.foreach_set(rgba.ravel());image.filepath_raw=str(WORK/'textures'/f'captain_hair_{channel}.png')
...
def build_beard(make, face_shape, hair, tree):
    ...
    obj=make('captain_rounded_jaw_beard',verts,faces,hair,'head')
    ...
    for side in [-1,1]:
        ...
        ob=make('captain_curved_mustache_'+str(side),vertices,faces,hair,'head')
        for face in ob.data.polygons:
            for loop in face.loop_indices:
