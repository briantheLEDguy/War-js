...
from captain_details import dress_captain
...
CAST = {'empire_field_captain': {'foundation': 'civic_humanoid_v2_m', 'race': 'empire', 'cloth': (.09,.135,.20), 'leather': (.22,.095,.040)}}
...
def build_provenance(source,master,recipe,body):
    ...
    inputs.update((WORK/name).resolve() for name in ('character-contract.json','foundation-provenance.json',
        'review/male-foundation.json','review/equipment-attachment-source.json'))
...
def build(kind, recipe):
    ...
    if kind=='empire_field_captain':
        sew_work_shirt(make,material,lambda point:morphology(point,race),rig)
        dress_captain(make,sweep,material,rig,{'cloth':cloth,'leather':leather,'seam':seam,'iron':iron,'brass':brass,'wood':wood})
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(WORK/'sources/frontier_sunmeadow_empire_field_captain.prefit.blend'))
    ...
    if race in ('dwarf', 'empire'):
        ...
        try:
            ...
            bpy.context.view_layer.update()
    rig['skeletonId']='sunmeadow_field_captain_humanoid_v1';rig['bindPoseId']='sunmeadow_field_captain_a_v1'
    rig['bodyFamily']='frontier_sunmeadow_field_captain_v1_m';rig['bodyVariant']='m'
...
if __name__ == '__main__':
    build('empire_field_captain', CAST['empire_field_captain'])
