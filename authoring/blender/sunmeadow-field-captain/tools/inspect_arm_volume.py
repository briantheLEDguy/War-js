...
SOURCE = WORK / (f'runtime/frontier_sunmeadow_empire_field_captain_lod{lod}.glb' if lod is not None
                 else 'sources/frontier_sunmeadow_empire_field_captain.blend')
...
body = max((o for o in bpy.context.scene.objects if o.type == 'MESH' and not o.hide_render),key=lambda o:len(o.data.vertices)) if lod is not None else bpy.data.objects['empire_field_captain_exposed_anatomy']
...
reference_points=np.array(reference['vertices'])*(1.0)*np.array([1,1,1])
...
report['maleFoundation']={'path':foundation.relative_to(WORK.parents[2]).as_posix(),'sha256':hashlib.sha256(foundation.read_bytes()).hexdigest(),
                           'authoringScale':1.0,'sections':reference_sections,
...
if min(reference_span_ratios)<.9 or max(reference_span_ratios)>1.1:
    raise RuntimeError('Exposed arm thickness differs more than ten percent from its actual male anatomical foundation')
