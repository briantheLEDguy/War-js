...
key=next((a.split('=',1)[1] for a in sys.argv if a.startswith('--asset=')),'frontier_sunmeadow_empire_field_captain')
...
rest=vertices();edge_groups={};submillimetre_only=[]
torso_indices=np.array(sorted({i for face in body.data.polygons if ('_woven_linen' in body.data.materials[face.material_index].name or body.data.materials[face.material_index].name=='captain_layer_leather_brigandine') for i in face.vertices if 1.12<rest[i,2]<1.45 and abs(rest[i,0])<.19} | {i for face in body.data.polygons if '_handled_brass' in body.data.materials[face.material_index].name for i in face.vertices}),dtype=int)
if not len(torso_indices):raise RuntimeError('Missing measured torso support surface')
sole_indices={}
hand_indices={}
for side in ('L','R'):
...
for material,entry in enumerate(body.data.materials):
    ...
    if not np.any(selected):
        submillimetre_only.append(entry.name);continue
...
for clip in doc['animations']:
    ...
    for seconds in sorted(times):
        ...
        samples.append({'seconds':seconds,'minimumHeight':float(world[lowest,2]),
                        'torsoSupportHeight':float(np.min(world[torso_indices,2])),
...
report={'model':source.name,'sha256':hashlib.sha256(raw).hexdigest(),'importedTriangles':imported_triangles,'status':'inspection_only','edgeMeasurementThresholdMetres':.001,'subMillimetreOnlyMaterials':submillimetre_only,'clips':records}
