...
def fit_locomotion(rig):
    ...
    for clip,duration,stride,lift in specs:
        print('Fitting captain ground contact: '+clip,flush=True)
...
def fit_stride_arms(rig):
    ...
    for clip,duration in [('idle',60),('walk',30),('run',20)]:
        print('Fitting captain arm counter-swing: '+clip,flush=True)
        ...
        for frame in range(duration+1):
            ...
            for side,sign,offset in [('L',1,0),('R',-1,.5)]:
                ...
                # A running elbow braces outside the fitted jerkin. The old
                # near-vertical pole folded the upper sleeve through its side.
                pole_width=1.1 if clip in ('walk','run') else .15
                _solve_chain(rig,side,target,rotation,('upper_arm','forearm','hand'),Vector((sign*pole_width,.65,-1.5)))
    ...
    for clip,duration in [('attack_melee',30),('attack_ranged',40),('cast',60)]:
        ...
        for frame in range(duration+1):
            ...
            for side,sign in [('L',1),('R',-1)]:
                ...
                if clip=='attack_melee':target.x+=sign*.08
                shoulder=rig.pose.bones['upper_arm_'+side].head
                delta=target-shoulder
                reach=(rig.pose.bones['upper_arm_'+side].length+rig.pose.bones['forearm_'+side].length)*.90
                if delta.length>reach:target=shoulder+delta.normalized()*reach
...
def fit_prone_contact(rig):
    ...
    print('Fitting captain forearm/boot contact: death',flush=True)
    ...
    skin_surface=_surface(bpy.data.objects['empire_field_captain_exposed_anatomy'])
    ...
    torso_surface=_surface(bpy.data.objects['shirt_continuous_tailored_surface'])
    ...
    for frame in range(61):
        ...
        for side,sign in [('L',1),('R',-1)]:
            ...
            brace=min(1,frame/12);brace=brace*brace*(3-2*brace)
            fall_brace=math.sin(math.pi*max(0,min(1,(frame-20)/18)))**2
            wrist.x+=sign*(.06*brace+.09*fall_brace)
