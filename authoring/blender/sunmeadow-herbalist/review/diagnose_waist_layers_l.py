...
(WORK/'review/waist-layer-diagnostic-l.json').write_text(json.dumps(result,separators=(',',':'))+'\n')
...
central=[r for rows in records.values() for r in rows if abs(r['point'][0])<.14 and r['point'][2]<.96]
for name in sorted({r['material'] for r in central}):
 rows=[r for r in central if r['material']==name];print('CENTRAL_BELOW_BELT',name,'count',len(rows),'z',min(r['point'][2] for r in rows),max(r['point'][2] for r in rows),'x',min(r['point'][0] for r in rows),max(r['point'][0] for r in rows),flush=True)
