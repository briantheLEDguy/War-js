...
from herbalist_details import dress_herbalist, dress_herbalist_tools
...
from herbalist_head import AUTHORING_INPUTS
...
CAST = {
    'dwarf_artisan': {'foundation': 'civic_humanoid_v2_m', 'race': 'dwarf', 'cloth': (.17, .23, .27), 'leather': (.24, .105, .046)},
        'empire_herbalist': {'foundation': 'civic_humanoid_v2_f', 'race': 'empire', 'cloth': (.52, .50, .41), 'leather': (.17, .092, .047)},
...
def build_provenance(source,master,recipe,body):
    inputs={Path(__file__).resolve(),source.resolve(),master.resolve(),
            (ROOT/'authoring/blender/frontier-population/foundations'/f"{recipe['foundation']}.quad-surface.json.gz").resolve(),
...
def build(kind, recipe):
    ...
    source = ROOT / 'authoring/blender/frontier-population/foundations' / f"body_{recipe['foundation']}.glb"
    ...
    race = recipe['race']
    for ob in meshes:
        for v in ob.data.vertices: v.co = morphology(v.co * (1.86/1.76), race)
    if race=='dwarf':
        body.data.update()
    ...
    for bone in rig.data.edit_bones:
        bone.head = morphology(bone.head * (1.86/1.76), race); bone.tail = morphology(bone.tail * (1.86/1.76), race)
    ...
    trousers = material(kind + '_wool', (.19,.225,.18), .95, textile=True)
    leather = material(kind + '_worked_leather', recipe['leather'], .72, textile=True)
    seam = material(kind + '_linen_seam', (.47,.45,.36), .94)
    ...
    dress_herbalist(make,sweep,material,rig,{'cloth':cloth,'leather':leather,'seam':seam,'iron':iron,'brass':brass,'wood':wood})
    # The tucked layers attach to the same waist frame as the belt. Blend
    # gently into the trunk and legs instead of letting leg weights pull
    # the panel tops and their bindings through the rigid leather.
    for garment in authored:
        hips_group=garment.vertex_groups.get('hips') or garment.vertex_groups.new(name='hips')
        for vertex in garment.data.vertices:
            if abs(vertex.co.x)>.30:continue
            z=vertex.co.z
            distance=max(.985-z,z-1.060,0)
            t=max(0,min(1,1-distance/.065));anchor=t*t*(3-2*t)
            if anchor<=0:continue
            fields=[(entry.group,entry.weight) for entry in vertex.groups]
            for group,weight in fields:garment.vertex_groups[group].add([vertex.index],weight*(1-anchor),'REPLACE')
            prior=next((weight for group,weight in fields if group==hips_group.index),0)
            hips_group.add([vertex.index],prior*(1-anchor)+anchor,'REPLACE')
    ...
    if kind in ('dwarf_artisan', 'greenskin_peat_worker'):
    ...
    belt_surfaces=[ClothSurface(o) for o in authored
                   if 'continuous_tailored_surface' in o.name or 'folded_work_apron' in o.name or o.name.startswith(('herbalist_fitted_smock_torso','herbalist_divided_smock_lappet','herbalist_bound_smock_opening','herbalist_lappet_turned_binding'))]
    ...
    def belt_hull(height):
        points=[];lower_plane=belt_z-.023;upper_plane=belt_z+.023
        ...
        for surface in belt_surfaces:
            # Use the complete belt-height slab, including narrow panel-top
            # bindings between the five profile stations.
            points.extend((p.x,p.y) for p in surface.points if lower_plane<=p.z<=upper_plane)
            for triangle in surface.triangles:
                for first,second in zip(triangle,triangle[1:]+triangle[:1]):
                    a,b=surface.points[first],surface.points[second]
                    for plane in (lower_plane,upper_plane):
    ...
    profile=[(.012,height) for height in heights]+[(.006,height) for height in reversed(heights)]
    ...
    if kind=='empire_herbalist':
        sew_work_shirt(make,material,lambda point:morphology(point,race),rig)
        dress_herbalist_tools(make,sweep,material,rig,{'cloth':cloth,'leather':leather,'seam':seam,'iron':iron,'brass':brass,'wood':wood})
    ...
    if race in ('dwarf', 'empire'):
        ...
        try:
            ...
            bpy.context.view_layer.update()
    rig['skeletonId'] = 'sunmeadow_herbalist_humanoid_v1'; rig['bindPoseId'] = 'sunmeadow_herbalist_a_pose_v1'
...
if __name__ == '__main__':
    selected = next((a.split('=',1)[1].split(',') for a in sys.argv if a.startswith('--assets=')), ['empire_herbalist'])
