...
WORK=Path(__file__).resolve().parents[1];master=WORK/'sources/frontier_sunmeadow_empire_herbalist.blend'
...
assert len([o for o in bpy.data.objects if o.name.startswith('herbalist_divided_smock_lappet')])==4
...
record={'masterSha256':hashlib.sha256(master.read_bytes()).hexdigest(),'shirtConnectedComponents':components,'shirtBoundaryLoops':loops,'shirtBoundaryValenceValid':True,'splitHemPanels':4,'packedImages':len(images),'modifiers':modifiers,'status':'passed','scope':'Continuous source shirt torso/axilla/sleeves; four intentionally separated smock hem panels and independently attached shoulder bridges; packed editable PBR inputs. Exact exported garment/tool contact measured separately.'}
