...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--assets',default='oak_pasture,oak_hedgerow,ash');parser.add_argument('--lods',default='0,1,2');parser.add_argument('--views',default='studio,ground');parser.add_argument('--accepted',action='store_true');args=parser.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
    records=[]
    for kind in args.assets.split(','):
        ...
        for lod in map(int,args.lods.split(',')):
          for view in args.views.split(','):
            ...
            model=(ROOT.parents[1]/'runtime' if args.accepted else ROOT/'runtime')/f'{key}_lod{lod}.glb';model_hash=sha(model);bpy.ops.import_scene.gltf(filepath=str(model))
            objects=[ob for ob in bpy.context.scene.objects if ob.type=='MESH']
            output=ROOT/'review'/f'{key}_lod{lod}_{view}{"_accepted" if args.accepted else ""}_reimport.png';bounds,camera=setup(objects,output)
            if view=='ground':
                camera.data.type='PERSP';camera.data.lens=48
                camera.location=(18,-29,1.7);target=Vector((0,0,5.8));camera.rotation_euler=(target-camera.location).to_track_quat('-Z','Y').to_euler()
                bpy.context.scene.render.resolution_x=1400;bpy.context.scene.render.resolution_y=1000
            bpy.ops.render.render(write_still=True)
            if lod==0 and view=='studio' and not args.accepted:bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'review'/f'{key}_reimport.blend'),compress=True)
            if sha(model)!=model_hash:raise RuntimeError('Export changed while its review image was rendering: '+model.name)
            records.append({'asset':key,'level':lod,'view':view,'model':str(model.resolve()),'model_sha256':model_hash,'image':str(output.relative_to(ROOT)),'image_sha256':sha(output),'bounds_blender':bounds,'camera_location':list(camera.location),'camera_lens':camera.data.lens,'renderer':'Cycles 24 samples, AgX, fixed daylight studio','decision':'pending'})
            (ROOT/'review'/f'{key}{"_accepted" if args.accepted else ""}_renders.json').write_text(json.dumps({'renders':[r for r in records if r['asset']==key],'visual_approval':False},indent=2)+'\n')
