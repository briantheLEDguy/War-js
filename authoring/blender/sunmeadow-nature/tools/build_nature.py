        ...
        for si in range(5):
            ...
            for ti in range(5 if ash else 8):
                ...
                for li in range(10 if not ash else 5):
                    ...
                    growth=Vector((math.cos(orient),math.sin(orient),rng.uniform(-.45,.90)))
                    leaf_length=rng.uniform(.34,.43);leaf_width=rng.uniform(.24,.30);leaf_twist=rng.uniform(-.08,.08)
                    retain=lod==0 or (lod==1 and leaf_index%3!=0) or (lod==2 and leaf_index%3==0)
                    scale=[1,1.18,1.52][lod]; leaf_index+=1
                    ...
                    if ash:
                        ...
                        for pair in range(2):
                            p=anchor.lerp(rachis_end,.20+pair*.34)
                            for side in [-1,1]:
                                direction=Vector((math.cos(orient+side*.85),math.sin(orient+side*.85),growth.z*.4))
                                leaf(g,p,direction,(.25-pair*.022)*scale,.12*scale,4+(li%2),'lance',lod,side*.025)
                        leaf(g,anchor.lerp(rachis_end,.77),growth,.25*scale,.11*scale,4,'lance',lod)
                    else:
