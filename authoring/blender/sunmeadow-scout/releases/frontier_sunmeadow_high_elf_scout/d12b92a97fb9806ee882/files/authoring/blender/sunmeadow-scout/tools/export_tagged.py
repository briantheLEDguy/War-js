"""Finish actual authored scout meshes with retained semantic part identity.
...
KEY='frontier_sunmeadow_high_elf_scout'
digest=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
master=WORK/'sources'/f'{KEY}.blend'
authored=WORK/'sources'/f'{KEY}.authored.blend'
bpy.ops.wm.open_mainfile(filepath=str(master))
if any(o.type=='MESH' and o.data.attributes.get('_SCOUT_PART') for o in bpy.context.scene.objects):
...
body=next(o for o in meshes if o.name=='high_elf_scout_exposed_anatomy')
parts={}
for part,obj in enumerate(meshes,1):
    attribute=obj.data.attributes.get('_SCOUT_PART') or obj.data.attributes.new('_SCOUT_PART','FLOAT','POINT')
...
mapping={'schemaVersion':1,'attribute':'_SCOUT_PART','authoringMaster':authored.relative_to(WORK).as_posix(),
...
base=body.data.copy();lods=[];pending=[]
for lod,ratio in enumerate([1,.58,.27]):
    ...
    identity_attribute=body.data.attributes['_SCOUT_PART']
    ...
    for mesh in doc['meshes']:
        for primitive in mesh['primitives']:
            accessor=doc['accessors'][primitive['attributes']['_SCOUT_PART']]
...
files,images=build_provenance(source,master,CAST['high_elf_scout'],body)
