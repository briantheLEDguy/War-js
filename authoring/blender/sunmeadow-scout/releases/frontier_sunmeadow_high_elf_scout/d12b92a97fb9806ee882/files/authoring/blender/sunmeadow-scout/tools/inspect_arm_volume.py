...
SOURCE = WORK / (f'runtime/frontier_sunmeadow_high_elf_scout_lod{lod}.glb' if lod is not None
                 else 'sources/frontier_sunmeadow_high_elf_scout.blend')
...
body = max((o for o in bpy.context.scene.objects if o.type == 'MESH' and not o.hide_render),key=lambda o:len(o.data.vertices)) if lod is not None else bpy.data.objects['high_elf_scout_exposed_anatomy']
...
reference_points=np.array(reference['vertices'])*(1.86/1.76)*np.array([.94,.97,1.055])
