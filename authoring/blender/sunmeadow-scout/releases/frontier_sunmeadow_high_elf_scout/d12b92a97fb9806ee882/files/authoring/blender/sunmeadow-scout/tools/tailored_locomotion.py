...
def fit_locomotion(rig):
    ...
    for clip,duration,stride,lift in specs:
        print('Fitting scout ground contact: '+clip,flush=True)
...
def fit_stride_arms(rig):
    ...
    for clip,duration in [('idle',60),('walk',30),('run',20)]:
        print('Fitting scout arm counter-swing: '+clip,flush=True)
...
def fit_prone_contact(rig):
    ...
    print('Fitting scout forearm/boot contact: death',flush=True)
    ...
    skin_surface=_surface(bpy.data.objects['high_elf_scout_exposed_anatomy'])
