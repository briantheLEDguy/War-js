"""Current-byte gates for all three scout exports and nine runtime clips."""
...
class ScoutExports(unittest.TestCase):
    def test_lowest_lod_retains_reviewed_boot_surfaces_and_weights(self):
        ...
        self.assertEqual(retention['inputMasterSha256'],sha(WORK/retention['inputMaster']))
        ...
        def actual_triangles(lod):
            ...
            names=[doc['nodes'][i]['name'] for i in doc['skins'][0]['joints']];result=collections.Counter()
            for mesh in doc['meshes']:
                for primitive in mesh['primitives']:
                    a=primitive['attributes'];part=values(a['_SCOUT_PART']);position=values(a['POSITION'])
    ...
    def test_shoulder_strips_follow_continuous_surface_routes(self):
        ...
        self.assertEqual(len(report['strips']),3)
        for strip in report['strips']:
            ...
            self.assertLessEqual(strip['maximumWidth'],.055,strip['object'])
    ...
    def test_actual_current_export_views(self):
        for lod in range(3):
            ...
            self.assertTrue({'front','head','side','rear','run_side','death:2'}<=views)
    ...
    def test_actual_semantic_equipment_attachments(self):
        ...
        for lod in range(3):
            ...
            self.assertEqual(len(report['links']),32)
            bridge_seams={f'{part}_{end}_jerkin_seam'
                          for part in ('scout_tapered_shoulder_yoke','scout_tapered_shoulder_yoke.001')
                          for end in ('front','rear')}
            bridge_seams|={f'{seam}_edge_{edge}' for seam in tuple(bridge_seams) for edge in range(2)}
            self.assertTrue(bridge_seams<={link['id'] for link in report['links']})
            for link in report['links']:
                if link['id'] in bridge_seams:
                    self.assertEqual(link['support']['part'],'scout_fitted_jerkin')
    ...
    def test_retained_female_anatomical_baseline(self):
        ...
        report=json.loads((WORK/'review/female-foundation.json').read_text())
        self.assertEqual(report['foundationFamily'],'civic_humanoid_v2_f')
        self.assertEqual(report['bodyVariant'],'f')
        ...
        self.assertEqual(report['morphScale'],[.94,.97,1.055])
    ...
    def test_master_continuity(self):
        ...
        self.assertTrue(report['fittedJerkin'])
        self.assertEqual(report['quiverSuspensionLoops'],2)
        self.assertGreaterEqual(report['packedImages'],15)

    def test_fitted_jerkin_and_suspended_equipment_clear_body(self):
        for lod in range(3):
