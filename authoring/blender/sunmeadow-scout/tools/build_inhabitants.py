...
CAST = {'high_elf_scout': {'foundation':'civic_humanoid_v2_f','race':'high_elf','cloth':(.080,.145,.105),'leather':(.16,.090,.040)}}
...
def build_provenance(source,master,recipe,body):
    inputs={Path(__file__).resolve(),source.resolve(),master.resolve(),
            (WORK/'publication-contract.json').resolve(),(WORK/'foundation-provenance.json').resolve(),
            (WORK/'review/equipment-attachment-source.json').resolve(),(WORK/'review/female-foundation.json').resolve(),
...
def _body_morphology(p, race):
    ...
    if race in ('high_elf', 'dark_elf'):
        ...
        ear=max(0,min(1,(abs(x)-.074)/.021))*math.exp(-((z-1.775)/.034)**2-((y+.032)/.045)**2)
        return Vector((x * slim+(.039*ear*(1 if x>0 else -1) if race=='high_elf' else 0), y * .97, z * (1.055 if race == 'high_elf' else 1.025)+(.026*ear if race=='high_elf' else 0)))
    return Vector(p)
...
def build(kind, recipe):
    ...
    trousers = material(kind + '_wool', (.105,.145,.115), .95, textile=True)
    ...
    if kind in ('dwarf_artisan', 'greenskin_peat_worker'):
        ...
        apron = make(kind + '_folded_work_apron', verts, faces, leather if kind != 'high_elf_scout' else cloth)
    ...
    belt_surfaces=[ClothSurface(o) for o in authored
                   if 'continuous_tailored_surface' in o.name or 'folded_work_apron' in o.name or o.name.startswith(('herbalist_fitted_smock_torso','herbalist_divided_smock_lappet'))]
    ...
    if kind=='high_elf_scout':
        sew_work_shirt(make,material,lambda point:morphology(point,race),rig)
        dress_scout(make,sweep,material,rig,{'cloth':cloth,'leather':leather,'seam':seam,'iron':iron,'brass':brass,'wood':wood},lambda p:morphology(p,race))
    ...
    if race in ('dwarf', 'empire', 'high_elf'):
        ...
        try:
            ...
            bpy.context.view_layer.update()
    rig['skeletonId'] = 'sunmeadow_scout_humanoid_v1'; rig['bindPoseId'] = 'sunmeadow_scout_a_pose_v1'
    ...
    for lod, ratio in enumerate([1,.58,.27]):
        ...
        final_master=WORK/'sources'/f'{key}_lod{lod}.blend'
        ...
        lods.append({'level':lod,'model':filename,'master':str(final_master.relative_to(WORK)),'masterSha256':digest(final_master),'sha256':digest(staged),'bytes':len(data),'tangentRepair':tangent_repair,
...
if __name__ == '__main__':
    selected = next((a.split('=',1)[1].split(',') for a in sys.argv if a.startswith('--assets=')), ['high_elf_scout'])
