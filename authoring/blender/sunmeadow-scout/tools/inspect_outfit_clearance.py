...
smock=[f for f,n in all_faces if n=='scout_forest_suede_jerkin']
gear=[f for f,n in all_faces if n.startswith('scout_tool_')]
obstacles=[f for f,n in all_faces if '_woven_linen' in n or '_wool' in n or n=='scout_forest_suede_jerkin' or '.body' in n]
...
foundation=WORK/'foundations/civic_humanoid_v2_f.quad-surface.json.gz'
foundation_hash=sha(foundation)
anatomy=json.loads(gzip.decompress(foundation.read_bytes()))
reference=np.array(anatomy['vertices'])*(1.86/1.76)
x,y,z=reference.T
ear=np.clip((np.abs(x)-.074)/.021,0,1)*np.exp(-((z-1.775)/.034)**2-((y+.032)/.045)**2)
reference=reference*np.array([.94,.97,1.055]);reference[:,0]+=.039*ear*np.sign(x);reference[:,2]+=.026*ear
reference_faces=[tuple(face) for face in anatomy['faces']]
closed={'retainedUnderlyingBody':require_closed(reference,reference_faces,'retained female body')}
for name,(subject,reference_surface) in sets.items():
    ...
    cloth=[f for f,n in all_faces if ('_woven_linen' in n or '_wool' in n or (name=='tool_clearance' and n=='scout_forest_suede_jerkin'))]
...
for name in sets:
    report={'model':source.name,'sha256':hashlib.sha256(raw).hexdigest(),'checkerSources':checker_hashes,'nativeContainmentCorrections':CONTAINMENT_DIAGNOSTICS,'importedTriangles':triangles,'probeVertices':len(probes[name]),'clips':records[name],
            'scope':'Actual exported surface intersections and closed-volume signed clearance, with a retained closed anatomical body beneath the real exported garment shells. Open neck/cuff surfaces are used for visible intersections but never as a face-plane inside/outside test. Complete jerkin and shoulder yokes are checked against shirt/trousers. Harness and quiver suspension loops intentionally contact their contents/support and are separate materials; actual carried quiver, arrows, bow and belt map case are checked against all worn/exposed body surfaces.','status':'measured','closedTopology':closed,'underlyingBody':{'path':foundation.relative_to(WORK).as_posix(),'sha256':hashlib.sha256(foundation.read_bytes()).hexdigest()},'falsePositiveDiagnostic':diagnostic,'realCrossingDiagnostic':real_diagnostic}
