...
def garments(family, race, make, morph, materials):
    ...
    points=[Vector(p) * (1.86/1.76) for p in source['vertices']]
    ...
    elbow={s:Vector((s*.360696,-.023474,1.270315)) for s in [-1,1]}
    direction={s:Vector((s*.129307,-.164421,-.136810)) for s in [-1,1]}
    ...
    for kind in ['shirt','trousers']:
        ...
        for face_index,face in enumerate(source['faces']):
            ...
            if kind=='shirt':
                if not 1.000<center.z<1.58 or any(hand(n) for n in names):continue
        ...
        if kind=='trousers':
            ...
            for index,p in enumerate(vertices):
                ...
                dx=p.x-side*.195631;dy=p.y+.03069
                radius=math.sqrt((dx/.050)**2+(dy/.042)**2)
                if radius>1:
                    p.x=side*.195631+dx*(1-blend+blend/radius)
                    p.y=-.03069+dy*(1-blend+blend/radius)
