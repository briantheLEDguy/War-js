...
from scout_head import build_head
from fitted_boot_details import sew_sole_welt
...
def tailor(kind, race, body, rig, make, sweep, morph, materials):
    ...
    family='mire_brutish_v1_m' if race=='greenskin' else 'civic_humanoid_v2_f' if kind in ('high_elf_scout','dark_elf_supply_officer') else 'civic_humanoid_v2_m'
    ...
    for side in [-1,1]:
        ...
        for z,rx,ry,cy in [(.155,.066,.065,-.013),(.215,.067,.061,-.008),(.285,.069,.06,-.008),(.304,.072,.063,-.008)]:
            for px,py in outline:
                ...
                verts.append(shape((side*.195631+rx*math.cos(a),cy-.02269+ry*math.sin(a),z)))
        ...
        sew_sole_welt(boot,make,materials['seam'],shape((0,0,.047)).z,side)
    ...
    if race in ('dwarf','empire','high_elf'):
        build_head(make,sweep,body,rig,materials,shape)
