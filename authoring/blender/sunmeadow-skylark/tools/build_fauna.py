...
def paint_material(kind,definition):
    ...
    if definition.get('hoof_profile')=='tapered_cloven_v2':colors[7]=[.16,.13,.105]
    ...
    if definition.get('hoof_profile')=='tapered_cloven_v2':rough=np.where(tile==7,.68,rough)
    ...
    for vertex in vertices:
        ...
        for limb,names in [('front_leg',['shoulder','forearm','carpal','pastern_front','hoof_front']),('hind_leg',['thigh','shin','hock','pastern_hind','hoof_hind'])]:
            ...
            if limb=='hind_leg':
                ...
                # Abdominal flank belongs to the pelvis above the stifle;
                # it must not rotate into the belly with the forward thigh.
                abdominal=1-blend(rows[0][1]-.16,rows[0][1]-.035,point.y)
                upper=blend(rows[2][2],rows[2][2]+.16,point.z)
                support*=1-definition.get('hind_abdominal_fade',0)*abdominal*upper
...
def mammal_geometry(kind,definition,lod):
    ...
    for side,label in [(1,'L'),(-1,'R')]:
        ...
        for limb,names,parent in [('front_leg',['shoulder','forearm','carpal','pastern_front','hoof_front'],'chest'),('hind_leg',['thigh','shin','hock','pastern_hind','hoof_hind'],'pelvis')]:
            ...
            if split and definition.get('hoof_profile')=='tapered_cloven_v2':
                add_deer_hoof(cage,foot,side,label,limb.removesuffix('_leg'),lod,atlas)
                continue
...
def build(kind,definition,rest_only=False):
    ...
    for level in range(3):
        ...
        if rest_only:bpy.ops.export_scene.gltf(filepath=str(target),export_format='GLB',use_selection=True,export_yup=True,export_normals=True,export_tangents=True,export_texcoords=True,export_skins=True,export_animations=False,export_vertex_color='NAME',export_vertex_color_name='AnatomicalTint',export_all_vertex_colors=False)
        ...
        lods.append({'level':level,'model':target.name,'sha256':sha(target) if rest_only else None,'bytes':target.stat().st_size if rest_only else 0,'triangles':triangles,'vertices':len(obj.data.vertices),'bounds_blender':{'min':minimum,'max':maximum},'parts':cage.parts,**texture_evidence})
    ...
    source_files=['source/anatomy.json',*['tools/'+name for name in ['build_fauna.py','quadruped_rig.py','motion.py','gait_curves.py','bird_geometry.py','hoof_geometry.py','stitched_skin.py','atlas_checks.py','surface_detail.py','texture_detail.py','texture_lods.py']]]
