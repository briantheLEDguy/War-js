...
def mammal_clips(rig,kind,definition):
    scale=definition.get('scale',1);fps=60;clips=[];reports=[];controls=definition.get('gait_controls',{})
    run_lift=controls.get('run_foot_lift',.15);run_crouch=controls.get('run_body_crouch',.043);run_bob=controls.get('run_body_bob',.010)
    ...
    for clip in definition['clips']:
        ...
        for frame in range(last+1):
            ...
            pelvis=rig.pose.bones['pelvis'];bob=(.006*math.cos(time*2) if clip=='walk' else -run_crouch+run_bob*math.cos(time*2) if running else .0015*math.sin(time))*scale
        ...
        if clip in ['walk','run','hop']:
            ...
            locomotion={'stride_m':stride,'stance_fraction':stance,'phase_offsets':gallop if running else walking,'speed_mps':stride/(stance*duration),'foot_lift_m':(run_lift if running else .065)*scale,'trajectory':'constant_velocity_stance_c1_return'}
