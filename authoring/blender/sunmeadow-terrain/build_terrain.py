"""Author Sunmeadow's sixteen landscape sectors from the shared survey grid.
...
STEP = SIZE / N
for folder in ['runtime', 'masters', 'review', 'textures/sunmeadow_terrain']:
...
def externalize(file):
    ...
    for mesh in doc['meshes']:
        for primitive in mesh['primitives']:
            ...
            if material_name=='Sunmeadow_limestone_road' and 'COLOR_1' in primitive['attributes']:
    ...
    for im in doc.get('images',[]):
        ...
        name=hashlib.sha256(raw).hexdigest()[:20]+'.png';(WORK/'textures/sunmeadow_terrain'/name).write_bytes(raw);im['uri']='../textures/sunmeadow_terrain/'+name
...
grass=material('Sunmeadow_short_grass','meadow_albedo.png');roadmat=material('Sunmeadow_limestone_road','road_albedo.png',True)
...
for chunk in SOURCE['terrain']['chunks']:
    ...
    for lod in range(3):
        bpy.ops.object.select_all(action='SELECT');bpy.ops.object.delete(use_global=False)
        surface=land(chunk,lod,grass);roads(chunk,roadmat)
