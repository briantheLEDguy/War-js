...
if(process.argv.includes('--publish')) {
  ...
  const models=path.join(root,'public/assets/models'),textures=path.join(root,'public/assets/textures/sunmeadow_terrain');
  const preview='authoring/blender/sunmeadow-terrain/review/terrain_overview_lod0.png';
  const writes=[];
  const copied=new Set();
  for(const asset of report) {
    const assetId=`prop.${asset.key.replaceAll('_','.')}`;
    const bp=structuredClone(template);
    Object.assign(bp,{assetId,displayName:asset.key.replaceAll('_',' '),sets:['sunmeadow_terrain'],runtime:{staticKey:asset.key},lifecycle:{status:'approved',reviewedBy:review.reviewedBy,reviewedAt:review.reviewedAt,notes:'Reviewed authored terrain; authoritative ground uses the exact source Float32 survey.'}});
    bp.output={model:asset.lods[0].model,artifactDir:'authoring/blender/sunmeadow-terrain/runtime'};
    bp.generator={kind:'copyExisting',copyFrom:`authoring/blender/sunmeadow-terrain/runtime/${asset.lods[0].model}`};
    bp.geometry.lods=asset.lods.map(lod=>({name:`LOD${lod.level}`,triTarget:lod.triangles,screenCoverageMin:[.2,.08,0][lod.level]}));
    bp.materials={master:'MM_SunmeadowTerrain',textureSet:'sunmeadow_terrain',channels:['baseColor','roughness','metallic','normal'],maxTextureResolution:1024};
    bp.qc={...bp.qc,maxDrawCalls:2,maxMeshObjects:2,maxTris:100000,maxFileSizeMb:16,requiresPreview:true};
    bp.collision={policy:'none',primitives:[]};
    bp.provenance={...bp.provenance,createdBy:'original_authored_blender_meshes',source:'authoring/blender/sunmeadow-terrain/build_terrain.py',promptIds:['sunmeadow_meadow_albedo_v1','sunmeadow_road_albedo_v1'],referencePackId:'original_sunmeadow_survey',aiStages:['art_direction','texture_source','mesh_authoring']};
