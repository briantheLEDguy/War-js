...
export function textureFile(uri, directory = work) {
  assert.match(uri, /^\.\.\/textures\/sunmeadow_terrain\/[a-f0-9]{20}\.png$/, 'Unexpected external terrain texture path');
...
export async function validateTerrain(directory = work) {
  ...
  for (const asset of report) {
    ...
    for (const lod of asset.lods) {
      assert.match(lod.model, /^frontier_sunmeadow_march_terrain_[0-3]_[0-3](?:_lod[12])?\.glb$/);
      ...
      for (const primitive of primitives.filter(primitive => doc.materials[primitive.material].name === 'Sunmeadow_limestone_road')) {
