...
| 3: RPG | All current combat, progression, inventory, gathering/crafting, quests and HUD behavior under server authority | Inventory rules pass seven browser-derived scenarios; owner-only bag/equipment replication, server reward receipts and revision-checked equipment commands pass native/two-client checks. Inventory UI, durable economy, crafting integration and the remaining RPG systems are pending |
