...
function generatedTs(roster) {
  const classJson = JSON.stringify(roster.classes.map(({ race, className, key }) => ({
    race, className, family: roster.races[race].family, key,
  })), null, 2);
  const variantJson = JSON.stringify(Object.entries(roster.bodyVariants).map(([bodyVariant, variant]) => ({
    bodyVariant, label: variant.label,
  })), null, 2);
  const armorJson = JSON.stringify(roster.armorSlots.map(({ slot, label, icon, regions }) => ({
    slot, label, icon, regions,
  })), null, 2);
...
const CLASS_SEEDS: Array<{ race: PlayableRace; className: string; family: string; key: string }> = ${classJson};
const BODY_VARIANTS: Array<{ bodyVariant: BodyVariant; label: string }> = ${variantJson};
const ARMOR_PARTS: Array<{ slot: EquipSlot; label: string; icon: string; regions: string[] }> = ${armorJson};
...
writeFileSync(GENERATED_TS_PATH, generatedTs(roster), "utf8");
