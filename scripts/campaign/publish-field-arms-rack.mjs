/** Publish the universal field arms rack after literal export and collision checks. */
...
const packagePath = 'authoring/blender/field-arms-rack', key = 'frontier_field_arms_rack';
const assetId = 'prop.frontier.field_arms_rack', name = 'Field Arms Rack';
const hash = bytes => createHash('sha256').update(bytes).digest('hex');
const jsonBytes = value => Buffer.from(JSON.stringify(value, null, 2) + '\n');

export async function publishFieldArmsRack(publish = false) {
  ...
  for (const file of ['scripts/campaign/publish-field-arms-rack.mjs', 'scripts/campaign/regional-publication-evidence.mjs',
  ...
  for (const lod of build.lods) {
    ...
    assert(!gltf.skins?.length && !gltf.animations?.length, 'Static rack must not add a character rig');
    ...
    await local('tools/review_rack.py', review.reviewerSha256);
  ...
  execFileSync(process.execPath, [inside(repo, `${packagePath}/tools/validate_rack.mjs`), '--check'], { cwd: repo, stdio: 'pipe' });
  ...
  const provenance = { createdBy: 'original_authored_mesh_control_cages', aiAssisted: true,
    ...
    referencePackId: 'original_universal_field_arms_rack_v1', similarityReview: 'not_required',
  ...
  const qcFiles = lods.map(lod => ({ file: lod.model.replace('.glb', '.qc.json'), bytes: jsonBytes({
    ...
    limitations: ['Static racked weapons; no pickup or equipment interaction claimed.'],
  }) }));
  ...
  const blueprint = { assetId, displayName: name, category: 'prop', version: '1.0.0', sets: ['universal_town_logistics'],
    ...
    materials: { master: 'MM_FieldArmsRack', textureSet: key,
    ...
    qc: { allowNonManifold: false, allowUvOverlap: true, maxTris: lods[0].triangles,
      maxFileSizeMb: 15, maxDrawCalls: 5, maxMeshObjects: 5, requiresSkinnedMeshes: false, requiresPreview: true,
  ...
  for (const [relative, bytes] of writes) {
    ...
    await fs.writeFile(`${target}.rack-pending`, bytes); await fs.rename(`${target}.rack-pending`, target);
  }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(JSON.stringify(await publishFieldArmsRack(process.argv.includes('--publish')), null, 2));
}
