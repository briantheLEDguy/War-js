/** Publish the universal field command table after literal export and collision checks. */
...
const packagePath = 'authoring/blender/field-command-table', key = 'frontier_field_command_table';
const assetId = 'prop.frontier.field_command_table', name = 'Field Command Table';
const hash = bytes => createHash('sha256').update(bytes).digest('hex');
const jsonBytes = value => Buffer.from(JSON.stringify(value, null, 2) + '\n');

export async function publishFieldCommandTable(publish = false) {
  ...
  for (const file of ['scripts/campaign/publish-field-command-table.mjs', 'scripts/campaign/regional-publication-evidence.mjs',
  ...
  for (const lod of build.lods) {
    ...
    await local('tools/review_table.py', review.reviewerSha256);
  ...
  execFileSync(process.execPath, [inside(repo, `${packagePath}/tools/validate_table.mjs`), '--check'], { cwd: repo, stdio: 'pipe' });
  ...
  const provenance = { createdBy: 'original_authored_mesh_control_cages', aiAssisted: true,
    ...
    referencePackId: 'original_universal_field_command_table_v1', similarityReview: 'not_required',
  ...
  const qcFiles = lods.map(lod => ({ file: lod.model.replace('.glb', '.qc.json'), bytes: jsonBytes({
    ...
    limitations: ['Static tableed weapons; no pickup or equipment interaction claimed.'],
  }) }));
  ...
  const blueprint = { assetId, displayName: name, category: 'prop', version: '1.0.0', sets: ['universal_town_logistics'],
    ...
    materials: { master: 'MM_FieldCommandTable', textureSet: key,
    ...
    qc: { allowNonManifold: false, allowUvOverlap: true, maxTris: lods[0].triangles,
      maxFileSizeMb: 15, maxDrawCalls: 6, maxMeshObjects: 6, requiresSkinnedMeshes: false, requiresPreview: true,
  ...
  for (const [relative, bytes] of writes) {
    ...
    await fs.writeFile(`${target}.table-pending`, bytes); await fs.rename(`${target}.table-pending`, target);
  }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(JSON.stringify(await publishFieldCommandTable(process.argv.includes('--publish')), null, 2));
}
