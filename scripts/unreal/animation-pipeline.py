...
def main():
    ...
    parser.add_argument("--from-stage", help="Start at this stage (defaults to the recipe's first stage)")
    ...
    args.from_stage = args.from_stage or names[0]
    ...
    if 'profileKey' in recipe:
        ...
        style_file.relative_to(TOOLS/'animation-recipes/styles')
        template=json.loads(style_file.read_text())
        if recipe['profileKey'] not in PROFILES or template['style']!=PROFILES[recipe['profileKey']][1] or template['locomotion']!=locomotion(template['style']):
            parser.error('Character recipe and reusable style template disagree')
        environment['WAR_ANIMATION_PROFILE']=recipe['profileKey']
    else:
