"""Fit the imported off-hand to the equipped hammer without changing body motion."""
import json
import math
...
(OUT / "grip-fit.json").unlink(missing_ok=True)
compression = unreal.load_asset("/Game/Imported/civic_battle_prelate_m/SourcePoseCompression")
if not compression:
    raise RuntimeError("Missing equipped rig's scale-aware compression settings")
...
for key, clip in receipt["clips"].items():
    ...
    poses = [unreal.AnimPoseExtensions.get_anim_pose_at_time(animation, min(i / 30, clip["duration"]), options)
    ...
    for i, pose in enumerate(poses):
        ...
        spacing = max(16., min(34., (r_palm - l_palm).dot(axis)))
        ...
        if discriminant >= 0:
            lower = max(12., axial - math.sqrt(discriminant))
            upper = min(45., axial + math.sqrt(discriminant))
            if lower <= upper:
        ...
        hips = local_bone("hips")
        # Linear travel belongs to CharacterMovement. Retain cycle sway/bob.
        if key in ("walk", "strafe", "slash", "slash_alternate", "jump", "jump_alternate"):
            first = unreal.AnimPoseExtensions.get_bone_pose(poses[0], "hips", unreal.AnimPoseSpaces.LOCAL).translation
            last = unreal.AnimPoseExtensions.get_bone_pose(poses[-1], "hips", unreal.AnimPoseSpaces.LOCAL).translation
            offset = (last - first) * (i / (count - 1))
            hips.translation = hips.translation - unreal.Vector(offset.x, offset.y, 0)
        tracks["hips"].append(hips)
