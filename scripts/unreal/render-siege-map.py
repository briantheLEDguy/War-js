"""Render the isolated siege offscreen without changing the saved map."""
import json
from pathlib import Path
import unreal

ROOT = Path(__file__).resolve().parents[2]
directory = ROOT / "artifacts/unreal/siege"
...
if not unreal.get_editor_subsystem(unreal.LevelEditorSubsystem).load_level(receipt["map"]):
    raise RuntimeError("Siege map is missing")
actors = unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
world = unreal.get_editor_subsystem(unreal.UnrealEditorSubsystem).get_editor_world()
for i in range(3):
    mesh = unreal.load_asset("/Game/WorldRebuild/Campaign_20260921_222411_608123/Meshes/prop_riftspire_war_brazier_"+str(i))
    unreal.log("WAR_SIEGE_BRAZIER_BOUNDS " + str(mesh.get_bounds()))
...
for view, eye_values, center_values in [
    ...
    ("outer-blockade", (13400, -450, 4460), (14200, 0, 4380)),
    ("inner-blockade", (16400, -450, 4450), (17300, 0, 4380)),
    ("inner-hall", (17600, -300, 4400), (19000, 0, 4380)),
    ("chain-control", (15100, -1800, 4400), (15600, -1400, 4300))]:
    eye, center = unreal.Vector(*eye_values), unreal.Vector(*center_values)
    settings.auto_exposure_bias = 3.0 if view == "inner-hall" else 1.0
    component.post_process_settings = settings
...
(directory / "siege-render-views.json").write_text(json.dumps(views, indent=2) + "\n")
unreal.log("WAR_SIEGE_RENDERED")
