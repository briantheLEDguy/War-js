...
const key = 'frontier_field_supply_chest', packagePath = 'authoring/blender/field-supply-chest';
...
test('the supply chest retains literal three-LOD exports, editable masters and matching GM collision', () => {
  ...
  expect(registry.staticProps[key]).toMatchObject({ assetId: 'prop.frontier.field_supply_chest',
  ...
  for (const lod of qc.lods) {
    ...
    expect(gltf.meshes).toHaveLength(4); expect(gltf.materials).toHaveLength(4);
    expect(gltf.images.length).toBeGreaterThanOrEqual(12);
  ...
  expect(construction.records.flatMap((record: { contacts: unknown[] }) => record.contacts)).toHaveLength(63);
...
test('all 36 keeps have the universal chest with supported skids and the published collider contract', async () => {
  ...
  for (const config of configs.filter(config => config.keeps.length)) {
    const zone = read(`public/assets/maps/${config.id}.json`), chests = owned(zone);
    expect(chests, config.id).toHaveLength(2);
    for (const keep of config.keeps) {
      const chest = chests.find(chest => chest.id === `${config.id}_delivered_${keep.realm}_supply_chest`)!;
      expect(chest).toBeDefined(); expect(chest.model).toBe(registry.staticProps[key].model);
      expect(chest.colliders).toEqual(metadata.colliders); expect(chest.colliderSpace).toBe('model');
      const y = chest.heightMode === 'absolute' ? chest.y ?? 0 : campaignGroundHeight(config, { ...chest, y: 0 });
      for (const dx of [-.493, .493]) for (const dz of [-.28, .28]) {
        const ground = campaignGroundHeight(config, { x: chest.x + dx, y, z: chest.z + dz });
        expect(Math.abs(ground - y), `${chest.id} skid ${dx}/${dz}`).toBeLessThan(.009);
      }
      total++;
    }
  }
...
for (const id of ['sunmeadow_march', 'cinderfen_outskirts']) {
  test(`${id}: supply chests clear actual nearby wall, stair and furnishing triangles`, () => {
    const zone = read(`public/assets/maps/${id}.json`), chests = owned(zone);
    const metadata = read(`${packagePath}/builder-metadata.json`).assets[key];
    ...
    for (const chest of chests) {
      ...
      const volume = new Box3(new Vector3(chest.x + lo[0], (chest.y ?? 0) + .02, chest.z + lo[2]),
        new Vector3(chest.x + hi[0], (chest.y ?? 0) + hi[1], chest.z + hi[2]));
      for (const prop of zone.props as PropSpawn[]) {
        if (prop.id === chest.id || prop.visible === false || !prop.colliders?.length
          || Math.hypot(prop.x - chest.x, prop.z - chest.z) > 40) continue;
        ...
        }), `${chest.id} clears ${prop.id}`).toBe(false);
      }
    }
  });
}
