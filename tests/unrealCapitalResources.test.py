...
class ResourceBindingsTests(unittest.TestCase):
    def test_preserves_source_node_and_visual_fields(self):
        ...
        self.assertEqual(len(source["resourceNodes"])-len(rows),2)

    def test_supply_profiles_remain_bound_to_source_crates(self):
        rows=resource_bindings(source_map())
        supplies=[r for r in rows if r["node"]["kind"] in ("scrap","ore")]
        self.assertEqual(len(supplies),4)
        self.assertTrue(all(r["profile"]=="aegis_crate_stack" for r in supplies))
        for row in rows:
            source=deepcopy(source_map())
            prop=next(p for p in source["props"] if p["id"]==row["prop"]["id"])
            prop["assetKey"]="unreviewed_replacement"
            with self.subTest(identity=row["node"]["id"]), self.assertRaises(ValueError):
    ...
    def test_missing_duplicate_or_replaced_visual_blocks_generation(self):
        for change in ("missing","duplicate","replace","model","move","scale","rotation","height"):
            source=deepcopy(source_map())
            ...
            elif change=="model": prop["model"]="training_dummy.glb"
            elif change=="move": prop["x"]+=1
            elif change=="rotation": prop["rotY"]=float("inf")
            elif change=="height": prop["y"]=True
