...
void UWarInventoryWidget::Refresh(AWarPlayerState* State)
{
    ...
    for (const auto& DisplayItem : DisplayItems)
    {
        ...
        FString Label = FString::Printf(TEXT("%02d  %s Ã—%d%s"), BagSlot + 1, *Name, Item->Quantity, bEquipped ? TEXT("  [Equipped]") : TEXT(""));
